...
K-POP Preset·Prompt Preview는 [K-POP Creation Roadmap](kpop-creation-roadmap.md)의 K1 `[진행 중]` 범위로 Studio에 연결했다. Dance·Easy Listening·Performance 선택은 Provider-neutral Prompt와 기존 `genre`로 컴파일되며 API DTO는 유지한다. Generation Options UI는 K2 후속 구현이고, Backend capability 전에는 BPM·언어 비율·Hook·Dance Break를 작동하는 제어로 노출하지 않는다.
