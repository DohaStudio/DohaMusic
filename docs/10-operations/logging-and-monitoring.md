...
> 현재 상태: **Backend·ACE-Step·Demucs 실험 로그 구현**
...
Demucs 실행기는 모델 load·분리 시간, Torch allocated/reserved, `nvidia-smi` 전체 사용량, process·system memory, process CPU peak·time과 두 Stem의 해시·크기·길이·신호 지표를 metadata JSON에 기록한다. Backend 로그에는 source 절대 경로나 오디오 내용을 남기지 않는다.
