...
`DOHAVOCAL_BASE_URL`과 `DOHAVOCAL_PAYLOAD_MAX_BYTES`는 application request·Project·settings snapshot으로 덮어쓸 수 없다. `0.2.0` read-only acquisition은 configured maximum과 caller 상한 중 작은 값을 적용한다. durable `PayloadLocator` persistence Service는 구현됐고 verified staging은 기존 `DOHA_ARTIFACT_STAGING_ROOT`를 재사용하는 local adapter authority를 승인했지만 adapter, Workspace Worker wiring, 인증, Artifact ingestion과 실제 Provider 운영은 활성화하지 않는다. transport 자동 retry도 없으므로 재시도 정책은 후속 orchestration 계약에서 별도로 결정한다.
