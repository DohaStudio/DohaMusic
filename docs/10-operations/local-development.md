...
Backend 설치에는 Mixer·K3.1 분석용 NumPy·SciPy, BS.1770 Integrated LUFS용 `pyloudnorm>=0.2,<0.3`, resource 측정용 psutil이 포함된다. pyloudnorm 0.2.0 pure Python wheel은 Windows·Python 3.12에서 설치·테스트했다. 기본 `DOHAMUSIC_AUDIO_MIXER=default`는 별도 AI·FFmpeg 설치 없이 실제 WAV를 합성한다. DSP 계약은 [Audio Quality Engine](../03-architecture/audio-quality-engine.md), K3.1 검증은 [EVAL-008](../../reports/evaluations/EVAL-008-audio-analysis-validation.md)을 따른다.
