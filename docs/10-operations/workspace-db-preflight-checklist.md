...
코드 기준선에서는 Runtime·Alembic online SQLite 연결의 FK 활성화와 startup 자동 Migration 기본 비활성화를 구현했습니다. source head는 `20260830_0025`, 실제 사용자 DB는 `20260810_0017`이며 대상 DB의 actual revision과 backup·rehearsal Gate를 새로 확인해야 합니다. 아래 Template의 체크 표시는 특정 실행 결과를 대신하지 않으므로 비워 둡니다.
