...
> 문서 상태: [사용자 승인 필요]
> 최종 수정일: 2026-08-06
> 관련 기능: SQLite 사용자 DB에 Workspace additive revision `20260806_0012` 적용
> 구현 상태: Runbook과 preflight 도구 준비 완료, 실제 사용자 DB Inventory·backup·Migration 미수행
> 관련 문서: [Preflight 체크리스트](workspace-db-preflight-checklist.md), [Backup·Rollback 정책](workspace-db-backup-rollback-policy.md), [Migration 전략](../07-database/database-redesign-migration-strategy.md), [검증 보고서](../../reports/validation/VALIDATION-WORKSPACE-DB-MIGRATION-PREFLIGHT.md)

## 1. 목적과 강제 중단 조건

이 Runbook은 현재 사용자 SQLite DB를 `20260801_0011`에서 `20260806_0012`로 안전하게 올리는 향후 승인 작업에만 사용합니다. 이 문서 작성 작업에서는 실제 DB를 읽거나 변경하지 않았습니다.
...
- Alembic 연결에서 `PRAGMA foreign_keys=ON`을 확인하지 못함
- 사용자의 최종 적용 승인이 없음
...
Alembic CLI의 기본 URL도 `backend/alembic.ini`의 같은 상대 문자열이지만, 앱은 시작 시 `upgrade_database()`에 Runtime의 `DATABASE_URL`을 전달합니다. 따라서 다음 순서로 하나의 대상 파일을 확정해야 합니다.
...
현재 앱은 lifespan 시작 시 자동으로 `upgrade head`를 실행합니다. 통제된 적용이 끝날 때까지 대상 DB를 가리키는 앱을 실행하면 안 됩니다. 자동 Migration 제어 기능은 이번 범위에 포함하지 않으며 별도 PR이 필요합니다.
...
| 20 | 앱 재시작 | 시작 로그와 health 정상 | 자동 추가 변경·오류 |
...
현재 Alembic `env.py`는 FK PRAGMA를 설정하지 않으므로 현 상태에서는 15단계를 통과할 수 없습니다. 이 문제가 별도 PR에서 검증되기 전 실제 적용 명령을 실행하지 않습니다.
