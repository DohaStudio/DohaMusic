...
- normal commit `3a97f5ec21ad44b5676c64aa8e77ed9975a87292`/tree `5cdf5d00a354895009d0ef2ce4db36eb661ef73d`의 run `35394949676`에서 Windows는 20 failed/283 passed다. owner provisioning은 성공했으나 TokenUser 자체가 RID 500인 built-in Administrator였다. SID profile을 완화하지 않고 RID 500 rejection negative를 추가했다. hosted Windows CI VM의 disposable 비관리자 계정으로 동일 native suite를 실행하도록 조정한다. 사용자 PC에서 OS account를 생성/삭제하지 않았다.
- 최신 focused 303 passed/0 skipped/0 failed (6.08s), direct 695 passed/0 skipped/0 failed (86.01s). 위 full 2207 결과는 이 RID negative 추가 전 로컬 실행이며 production/runtime source와 fixture 구현은 이후 변경되지 않았다. 추가 negative 및 CI launcher 검증은 별도 결과로 구분한다. hosted launcher PowerShell parse/정적 Gate PASS이며 launcher 자체는 사용자 PC에서 실행하지 않고 새 exact-head Windows CI로 확인한다.
