...
3. claim lease는 활성 invocation이 heartbeat로 소유한다. DB transaction은 network/I/O를 감싸지 않는다. 현재 lease expiry failure를 resume로 가장하지 않으며 lease 간 재진입에는 `DURABLE_EXECUTION_HANDOFF_REQUIRED`가 필요하다.
...
세부 lifecycle, retry, cancellation과 crash matrix는 [DohaVocal Worker Reconciliation Contract](../03-architecture/dohavocal-worker-reconciliation-contract.md)를 authoritative source로 삼는다.

## Consequences

- concrete wiring은 공개 상태 enum, DB schema, API 또는 Provider 계약을 임의로 확장할 수 없다.
- bounded single-invocation wiring은 가능하지만 장시간 handoff, durable locator, payload downloader와 Completion adapter가 없으면 end-to-end production completion을 완료로 선언할 수 없다.
- Provider Job binding과 Completion idempotency를 재사용해 crash replay의 중복 side effect를 제한한다.
- 현재 process-local locator와 lease expiry failure의 한계를 문서와 운영 상태에서 계속 드러낸다.
