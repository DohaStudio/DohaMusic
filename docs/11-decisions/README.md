...
| [ADR-050](ADR-050-verified-durable-staging-authority.md) | locator-derived local publish·restart adoption·cleanup의 verified byte authority | 승인, adapter 구현 대기 |
...
- [ADR-050 — Verified Durable Staging Authority](ADR-050-verified-durable-staging-authority.md): 기존 staging root와 PayloadLocator schema에 locator-derived exclusive local publish, full open verification과 orphan adoption을 결합하고 새 schema가 불필요함을 확정한다.
