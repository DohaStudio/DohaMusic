...
| [ADR-100](ADR-100-admission-journal-transaction-owner-foundation.md) | exact AdmissionAttempt와 external journal CAS/final guard/commit outcome의 one-shot transaction ownership | #187 merged·reconciliation/admission 아님 |
...
| [ADR-103](ADR-103-durable-admission-production-composition-foundation.md) | exact provider graph·caller/external transaction을 ceremony scope에 결합하는 strict composition/activation gate | Foundation Draft·production activation unavailable |
