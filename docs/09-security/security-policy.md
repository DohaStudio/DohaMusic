...
#163 issuance-integrity Foundation은 merged됐으나 production current eligibility는 활성화하지 않았다. [ADR-078](../11-decisions/ADR-078-deployment-verifier-current-status-lifecycle-contract.md)의 별도 Draft Contract는 independently provisioned pin, serialized fresh journal/private currentness witness, normal cross-signature와 compromise 외부 재지정 및 mismatch deny를 정의한다. 이는 실제 key provisioning/서명 ceremony 또는 store/claim/binding 구현 evidence가 아니다. 기존 ADR-077은 그대로 보존한다.
