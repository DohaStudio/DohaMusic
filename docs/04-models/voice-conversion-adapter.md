...
Seed-VC 출력은 길이·sample rate·channel·RMS·peak·무음·clipping을 자동 검사한다. 현재 구현에서 clipping은 metadata 경고이며 Job 실패 조건은 아니지만, 이는 운영 허용을 뜻하지 않는다. Phase 4.5에서 float benchmark 3/3의 full-scale 부근/초과 위험을 확인했고 현재 PCM16 export 경로는 같은 입력으로 재검증되지 않았다. pre/post resample·export 측정과 headroom/limiter 회귀 기준이 확정될 때까지 `seed_vc`를 운영 Pipeline에 연결하지 않는다. Phase 5 기술 검증은 `MockVoiceConverter`만 사용한다.
