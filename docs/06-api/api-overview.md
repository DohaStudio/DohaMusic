...
> 현재 상태: **Legacy Backend Foundation 구현 완료 / Workspace Job 계약·source schema·Index·Cursor·Repository keyset·Service state/cancel/retry 기반 완료·API 미구현**
...
DohaStudio Common Specification과 Asset 중심 DB Redesign을 기준으로 `/api/v1` Workspace REST API를 별도로 설계했다. 공통 기반과 Workspace Resource Endpoint 25개를 구현했다. Workspace Job은 Aggregate·상태·입출력 role·Provider·claim/lease·completion 경계를 확정하고 revision `20260810_0017`의 schema·Index를 실제 사용자 DB에 적용했으며 Cursor·Owner/Workspace Repository keyset page와 생성·상태·취소·재시도 Service 기반을 구현했다. Worker·Completion Unit of Work·API는 미구현이며 Job API는 0/5다. 목표 계약은 16개 그룹·64개 Method/Path 조합이며 나머지 39개는 `[계획]`이다. 아래 현행 API와 payload는 그대로 유지한다.
