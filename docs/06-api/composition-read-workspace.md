...
D1-A·D1-Transition·D1-B Frontend 완료와 canonical Track·Clip persistence·trusted duration·revision-safe idempotency·WorkingComposition Service/API는 source·격리 fixture 범위다. 실제 사용자 DB `0017 → 0022` migration·Bootstrap·data backfill, 실제 인증 principal, 실제 사용자 Workspace Snapshot E2E, Frontend Clip Editing·Composition commit·canonical Section과 typed Mixer는 계속 `NOT IMPLEMENTED`다. mutation 계약은 [WorkingComposition Product API](working-composition-api.md)를 따른다.
