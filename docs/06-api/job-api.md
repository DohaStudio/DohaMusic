...
현재 Resource API는 25/64, Workspace Job API는 0/5다. 역할·Workspace scope·cancellation marker·claim/lease Column과 Index는 source revision `20260810_0017`에 구현했다. 실제 DB 적용·Cursor·Repository keyset·Service state machine·Worker와 completion Unit of Work를 구현·검증하기 전에는 이 API를 완료로 표시하지 않는다.
