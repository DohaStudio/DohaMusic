...
> 구현 상태: `/api/v1` 공통 기반·명시적 Bootstrap 도구·HMAC Cursor와 Workspace·MusicProject·ProjectAsset Resource Endpoint 11개 구현, 나머지 Endpoint·Adapter·Redirect·OpenAPI 파일 미구현
...
목표 API는 `/api/v1` 아래 Workspace·Project·Asset·AssetVersion·Artifact·CompositionSnapshot·Job 중심으로 통합합니다. 공통 Router·응답·request ID·오류와 HMAC Cursor 기반에 Workspace·MusicProject·ProjectAsset Resource Route 11개를 연결했으며 기존 Runtime Endpoint의 동작이나 status는 변경하지 않습니다.
