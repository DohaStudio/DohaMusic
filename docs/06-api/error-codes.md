...
API 오류는 `{ "error": { "code", "message" } }` 형식이다. 비동기 Worker 오류는 `generation_jobs.error_code`와 안전한 사용자 메시지로 기록되고 Job은 `FAILED`가 된다. 내부 스택·로컬 절대 경로·prompt·lyrics는 응답에 노출하지 않는다.
