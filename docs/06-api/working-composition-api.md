...
> 관련 문서: [Service Architecture](../03-architecture/working-composition-service.md), [Workspace REST API](workspace-rest-api-contract.md), [Composition Read](composition-read-workspace.md)
...
| 409 | `TRACK_NOT_EMPTY`, `CLIP_OVERLAP` | Product invariant 충돌 |
...
이 Router는 APIRoute 13개, OpenAPI Path 12개, Operation 13개이며 operation ID 중복은 0개다. 전체 application은 Route 90개, APIRoute 86개, `/api/v1` APIRoute 45개, OpenAPI Path 67개, Operation 88개다. 기존 Legacy Pipeline의 GET/HEAD 병합 route 두 곳에서 발생하던 global duplicate operation ID warning 2종은 이 작업 범위에서 변경하지 않았으며, 새 WorkingComposition operation ID와의 충돌은 0개다.

이 13개는 Product API이므로 Workspace Resource Endpoint 30/64 분모에는 포함하지 않는다.
