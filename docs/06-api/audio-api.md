...
기존 API는 한 요청에서 단일 WAV를 검증하고 `READY` Profile까지 즉시 생성한다. 별도 `/api/voice-enrollments` API는 WAV/WebM/Ogg, 최대 10개 sample, sample별 기본 품질, 설명, 사전 validation, create·upload·submit idempotency와 명시적 대표 reference 제출을 지원한다. upload resume와 동의 철회 API는 없으며 기존 단일 WAV path는 전환 fallback으로 유지한다.
