...
현재 Timeline Foundation은 Clip rectangle, waveform 추출·canvas editor, Section marker, multi-track sync engine, advanced Mixer를 포함하지 않는다. BPM·meter authority가 없으므로 ruler는 초·`mm:ss`만 사용하며 Bar/Beat는 후속 계약이다.
