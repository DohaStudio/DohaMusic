...
개인이 가사·작곡·가창·믹싱을 모두 수행하기 어렵고 공개 생성 모델마다 한국어 가창, 개인 GPU, 음성 권리 조건이 다르다. DohaMusic은 교체 가능한 사전학습 Provider를 안전한 비동기 Pipeline으로 연결하고, 사용자가 AI 초안을 편집·선택·승인하는 재현 가능한 개인 창작 흐름을 제공한다.

- 본인 또는 명시적으로 동의받은 음성만 사용하고 모델·데이터 출처·설정·Version·사용자 선택과 오류를 추적한다.
- 공개 사전학습 모델과 Adapter를 우선하며 기반 음악 생성 모델의 자체 사전학습은 현재 범위가 아니다.
- RTX 3060 Ti 8GB에서 순차 실행과 offloading 가능성을 우선 검증하되, 측정하지 않은 성능을 보장하지 않는다.
- 실시간 라이브 가창, 결제, 대규모 다중 사용자 운영과 상용 전문 DAW 전체 기능 parity는 현재 범위가 아니다.
- Timeline·Track·Clip·Mixer는 CURRENT MVP의 비목표였지만 장기 TARGET에는 포함한다. DohaMusic의 AI 공동 창작·불변 Version·평가 기반 수정에 필요한 범위를 단계별로 검증한다.
- 타인 음성의 무단 복제나 사칭은 CURRENT와 TARGET 모두에서 지원하지 않는다.
...
현재 Timeline Foundation은 Clip rectangle, **Clip source** waveform·canvas editor, Section marker, multi-track sync engine, advanced Mixer를 포함하지 않는다. Master/Mix Waveform만 존재한다. BPM·meter authority가 없으므로 ruler는 초·`mm:ss`만 사용하며 Bar/Beat는 후속 계약이다.
...
기존 ADR-002, ADR-012, ADR-017, ADR-028~035의 모듈·Orchestrator·Frontend·Provider·Workspace 경계를 유지한다. [ADR-040](../11-decisions/ADR-040-canonical-track-clip-working-composition-authority.md)은 Track·Clip identity, mutable WorkingComposition, edit/undo와 불변 Snapshot commit 계약을 확정한다. `CompositionEvaluationRun` 수명주기 또는 Learning opt-in은 구현 전에 각각 새 ADR 필요성을 다시 검토한다.
