...
Pipeline 필드와 보존 규칙은 [Pipeline 테이블](pipeline-tables.md)을 따른다. `20260801_0011`은 `voice_samples.quality_metrics`와 `idempotency_records`, `20260806_0012`는 Workspace Table 21개, `20260807_0013`~`20260808_0015`는 Workspace·Project·ProjectAsset·Asset keyset Index를 additive하게 추가했다. `20260809_0016`은 별도 내부 `artifact_storage_locations` Catalog Table 하나, `20260810_0017`은 Job role·scope·cancel·claim/lease Column과 Index 6개를 추가했으며 모두 실제 사용자 DB에 적용했다.

## Asset 중심 목표 DB — [진행 중]

DohaStudio Common Specification을 기준으로 Workspace·MusicProject·Asset·AssetVersion·Artifact·CompositionSnapshot·Job 중심의 21개 Workspace Entity와 별도 `ArtifactStorageLocation` Entity를 구현했다. source metadata와 실제 사용자 DB는 `20260810_0017`의 총 36개 Application Table이다. Workspace Job role·Workspace scope·cancellation·claim/lease Column과 Index는 실제 DB 적용까지 완료했지만 Cursor·Worker·API는 미구현이다. 신규 Workspace Table backfill과 Runtime 전환도 미수행이며 현행 Runtime Table 14개를 변경하거나 제거하지 않는다.
