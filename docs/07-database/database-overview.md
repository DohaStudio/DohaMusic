...
DohaStudio Common Specification을 기준으로 source Entity 33개와 별도 `ArtifactStorageLocation` Entity를 구현했다. source metadata는 `20260903_0026` 기준 48개 Application Table이고 실제 사용자 DB는 `20260810_0017` 기준 36개다. `0021`과 `0022`는 Table 수를 늘리지 않고 각각 nullable trusted Artifact duration과 versioned idempotency completion result를 추가하고 `0023`은 PayloadLocator 1개, `0024`는 Working Preview 4개 table, `0025`는 세 Clip table의 Gain column, `0026`은 같은 table의 Fade column을 추가한다. Workspace Resource API 30개와 별도 Product aggregate API, Job API 5개도 구현했다.
