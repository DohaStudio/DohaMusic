...
SQLAlchemy 2.x ORM을 사용하고 Alembic이 스키마 버전을 관리한다. 소스 head와 실제 사용자 DB는 Workspace Job schema·Index를 추가한 `20260810_0017`이다. 애플리케이션 startup의 자동 Migration은 기본 비활성화이며 `DOHAMUSIC_AUTO_MIGRATE=true`를 명시한 경우에만 기존 `upgrade head`를 실행한다. 사용자 DB에는 opt-in을 사용하지 않고 [Workspace DB Migration Runbook](../10-operations/workspace-db-migration-runbook.md)의 승인 절차를 따른다.
...
| CURRENT Workspace/Domain | Workspace 도메인 Entity/Table 21개와 내부 Storage Catalog 1개 | ORM·Repository·Service와 additive schema가 구현됨. 기존 Runtime을 대체하지는 않음 | [Workspace DB 구현 상태](database-redesign-overview.md), [Workspace Table Definition](database-redesign-table-definition.md) |
...
| TRANSITION | CURRENT Runtime에서 Workspace 중심 source of truth로 단계적 전환 | revisions `0012`~`0017` 적용 완료, 데이터·Runtime 전환 단계 미수행 | [Migration 전략](database-redesign-migration-strategy.md) |
...
| Workspace domain entities/tables | 21 | `backend/models/workspace/`의 도메인 Entity 21개와 revision `20260806_0012`의 `create_table` 21개 |
| Workspace storage catalog | 1 | `ArtifactStorageLocation` / `artifact_storage_locations`, revision `20260809_0016` |
| Application metadata tables | 36 | Runtime 14 + Workspace domain 21 + Catalog 1 |

`20260807_0013`~`0015`는 신규 Table이 아니라 Workspace·Project·ProjectAsset·Asset keyset Index를 추가한다. `20260810_0017`도 신규 Table 없이 기존 Workspace Job Table에 scope·role·cancel·claim/lease Column과 Index 6개를 추가한다.
...
`Asset`, `AssetVersion`, `Artifact`, `CompositionSnapshot`과 공통 `Job`은 목표 ORM과 `0012`~`0017` additive migration으로 실제 사용자 DB에 적용됐고 Runtime source of truth는 전환하지 않았다. Workspace·MusicProject·ProjectAsset·Asset·AssetVersion·Artifact·CompositionSnapshot·Job Resource API 30개를 구현했다. CompositionSnapshot은 불변 aggregate·자동 version·Cursor·Idempotency Service 기반과 공식 목록·생성·상세 API 3개를 제공하며 Workspace Job API는 5/5다. 목표 모델에서 Snapshot은 최신 Asset ID가 아니라 특정 Lyrics·Music·Vocal·Stem AssetVersion과 processing chain·mix settings·Provider·모델 버전을 불변으로 참조한다.
