...
> 구현 상태: 목표 Workspace Entity 21개·Catalog·Job schema·Index `0017` 실제 DB 적용, Resource API 25개 완료·Job API 0/5; 실제 Bootstrap·backfill·dual write 미수행
...
- role, direct Workspace scope, cancellation marker, Worker claim·lease Column과 Job keyset·Worker Index는 [Workspace Job Foundation](../03-architecture/workspace-job-foundation.md)에 따라 revision `20260810_0017`로 실제 사용자 DB에 적용했습니다. scope·role은 기존 row 보존을 위한 nullable staging이며 실행 로직은 미구현입니다.
