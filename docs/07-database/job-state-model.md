...
Retry는 terminal 원본의 상태를 되돌리지 않고 새 Job을 생성한다. 같은 Job 안의 Worker ownership generation과 공개 retry lineage를 구분한다. CURRENT runtime은 lease 만료 running Job을 자동으로 queued에 되돌리지 않고 retryable failure로 종료하며, TARGET reclaim은 아래 별도 계약을 따른다.
