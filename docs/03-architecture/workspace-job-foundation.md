...
> 문서 상태: [완료: 계약·Entity·Migration·실제 DB 적용·Index·Cursor·Repository keyset] / [미구현: Service state machine·Worker·API]
> 최종 수정일: 2026-08-10
...
현재 `jobs`, `job_inputs`, `job_outputs`, `model_usages` Entity·Repository·Service 기반에 role·Workspace scope·cancellation marker·claim/lease Column과 검증된 Index를 추가하고 revision `20260810_0017`을 실제 사용자 DB에 적용했다. Job HMAC Cursor와 Owner·Workspace scope Repository keyset·Service page 기반도 구현했다. 공개 Job API 5개, Service state machine, Worker claim·lease 로직과 completion Unit of Work는 아직 구현하지 않았다. Application Table은 36개이며 현행 Runtime Table 14개가 source of truth다.
...
`jobs.workspace_id`, `job_inputs.input_role`, `job_outputs.output_role`은 기존 row 보존을 위한 nullable staging이며 실제 사용자 DB에도 이 계약으로 적용했다. Job Cursor와 Repository keyset·Service page 기반은 구현했다. 의미 기반 role backfill, `NOT NULL` 강화, Service state machine·Worker·Provider 호출·공식 API 5개는 아직 구현하지 않았다.
