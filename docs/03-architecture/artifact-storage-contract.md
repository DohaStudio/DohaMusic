...
Provider wire result는 이 trusted ingestion의 입력 authority가 아니다. [Provider Result Ingestion Contract](provider-result-ingestion-contract.md)는 durable binding·Workspace Job·role·Manifest·lineage를 검증한 내부 candidate만 만들며, `payload_present=false`와 `checksum_scope=metadata_descriptor`인 결과는 binary와 structured ingestion 모두 거부한다. [Trusted Payload Locator / Resolver Contract](trusted-payload-locator-resolver-contract.md)은 별도의 DohaMusic-owned staging locator를 실제 bytes에서 검증한다. verified stream을 path 없이 Artifact ingestion에 넘기는 계약은 [DohaVocal Verified Staged Artifact Completion](dohavocal-verified-staged-artifact-completion.md)에 확정했고 본 Foundation branch에 구현했다. production rights adapter와 Worker wiring은 후속이다. Provider-side Artifact·AssetVersion ID와 descriptor checksum은 실제 Payload authority가 아니며 fake `temporary_path` 또는 arbitrary URI를 만들거나 해석하지 않는다.
