...
격리 SQLite fixture에서 revision race exactly-one success, initialize unique race, inverse replay fidelity, 동일 canonical ID 복원, atomic Track reindex, current source eligibility, exact split geometry와 Loop phase continuity, Clip Gain·Fade inheritance/divergence, overlap과 forced rollback을 검증했다. 현재 repository single head는 세 Clip table에 Loop geometry를 추가한 `20260905_0027`이다. 실제 사용자 DB·media·Provider·GPU에는 접근하지 않았다.
...
Preview manifest schema 4는 Clip별 `timeline_duration_us`, `loop_enabled`, `loop_phase_us`, Gain과 Fade를 immutable하게 pin한다. renderer는 source window, loop expansion, static Gain, full-timeline Fade, placement, Track mix 순으로 처리한다. persisted schema 1~3은 `D = source_out - source_in`, loop false, phase 0으로 읽고 schema 4의 필수 geometry 누락은 fail closed한다. 이후 Working mutation은 기존 manifest·Artifact를 바꾸지 않으며 Preview·Commit을 자동 생성하지 않는다. Waveform은 source-shape projection을 유지해 Loop·Gain·Fade mutation으로 decode cache나 peak를 다시 만들지 않는다.
