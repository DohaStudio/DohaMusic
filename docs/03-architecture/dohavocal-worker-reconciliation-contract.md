...
durable `PayloadLocator`, verified durable byte staging과 acquisition orchestration foundation은 구현됐다. 본 Foundation branch는 [ADR-074](../11-decisions/ADR-074-dohavocal-verified-staged-artifact-completion-authority.md)의 stream handoff·output target·atomic locator handoff·Session-aware scope/binding port를 구현했다. production current-rights adapter, reclaim wiring, production authentication, background daemon과 실제 DohaVocal model/GPU 실행은 별도 후속 의존성이다.
