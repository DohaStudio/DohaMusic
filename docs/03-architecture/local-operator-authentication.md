...
> Bootstrap 연계(2026-09-18): #162 merged [ADR-076](../11-decisions/ADR-076-product-deployment-bootstrap-authority.md)의 외부 deployment designation/approval/custodian과 target human authentication은 별개다. WebAuthn 성공만으로 기존 owner claim, Local Operator를 root/Rights issuer로 승격하는 것은 금지한다. 별도 Draft [issuance-integrity verifier](bootstrap-issuance-integrity-verifier.md) 결과도 human proof가 아니다. [Bootstrap architecture](product-deployment-bootstrap-authority.md)는 미구현 adapter의 future trusted internal-principal requirement를 정의하며 기존 authentication 계약·algorithm·TTL은 변경하지 않는다.
