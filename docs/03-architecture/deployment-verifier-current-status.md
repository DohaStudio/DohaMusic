...
Bootstrap 최신 Track: #164~#170/#172~#180은 merged다. [ADR-092 fresh journal-lineage observation](../11-decisions/ADR-092-fresh-journal-lineage-observation-foundation.md)은 별도 journal transaction의 complete history/head를 held lineage/pin/lease와 결합한다. [ADR-093 scoped provisioning authority](../11-decisions/ADR-093-scoped-provisioning-authority-verification-foundation.md)는 governance가 승인한 exact purpose/scope/domain과 root-signed rotation/revocation history를 검증한다. [ADR-094 authenticated authority source](../11-decisions/ADR-094-authenticated-scoped-provisioning-authority-source-foundation.md)는 fixed private source의 identity/custody와 same-handle reread를 그 검증에 결합한다. 결과는 currentness witness/admission이 아니며 production ports는 unavailable, schema/migration과 Alembic 0037은 유지된다.
