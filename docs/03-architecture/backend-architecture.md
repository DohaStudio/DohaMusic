...
Artifact Storage의 목표 경계는 `Router → Artifact Application Service → Storage Resolver·trusted ingestion → Artifact·Catalog Repository`다. Artifact Entity는 무결성 Metadata만 유지하고 별도 내부 `ArtifactStorageLocation`과 `artifact_storage_locations` Catalog가 backend·domain·canonical storage key를 소유한다. Catalog Entity와 revision `20260809_0016`은 실제 사용자 DB에 적용했으며 Catalog row는 0개다. Catalog 조회 전용 Repository와 `backend.storage.ArtifactStorageResolver`를 구현해 `local`·locator version `1`·승인 domain root·canonical key·containment·symlink/reparse·regular file·descriptor identity를 검증한다. owner와 retention은 향후 Artifact Application Service 책임이며 Resolver를 Router에서 직접 호출하지 않는다. trusted ingestion·physical checksum·MIME 검증·Range와 Artifact API 3개는 아직 구현하지 않았다. 공개 Artifact POST는 없으며 후속 trusted ingestion만 실제 bytes에서 SHA-256·크기·MIME를 검증한 뒤 등록한다.

이 계층은 additive 구현이다. 기존 Runtime Entity 14개와 Runtime Repository·Service·API는 변경하지 않았고 계속 운영 source of truth다. Workspace Application Service는 별도 namespace로 완료했고 `/api/v1` 공통 기반, 명시적 Bootstrap 도구, HMAC Cursor와 Workspace·MusicProject·ProjectAsset·Asset·AssetVersion Resource Endpoint 19개를 추가했다. AssetVersion Router는 생성·최신순 목록·상세만 공개하고 Service가 단조 증가 번호와 transaction을 소유한다. PATCH·DELETE는 제공하지 않는다. Resource API 진행도는 19/64, Artifact API는 0/3이며 Artifact 이하 45개 Endpoint와 trusted ingestion·backfill·dual write·Legacy 제거는 미구현이다.
