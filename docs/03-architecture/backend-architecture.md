...
> 현재 상태: **Phase 2 Provider·ACE-Step Adapter 경계 구현**

```text
backend/
├── alembic/       # DB 마이그레이션
├── ai/             # 생성 인터페이스, Provider Factory, Mock·ACE-Step Adapter
...
애플리케이션 시작 시 저장소 디렉터리를 준비하고 Alembic을 `head`까지 적용한 뒤 Repository, Service, Provider Factory와 Worker를 조립한다. 기본 Mock은 즉시 준비되며 ACE-Step은 애플리케이션 시작 시 공식 패키지나 모델을 로드하지 않는다. 실제 ACE-Step Job에서만 격리 runtime 경로를 검증한다. 공개 오류는 공통 `{ "error": { "code", "message" } }` 형식으로 변환하며 내부 예외 정보는 로그에만 남긴다.
