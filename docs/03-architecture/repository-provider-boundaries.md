...
> 관련 문서: [시스템 아키텍처](system-architecture.md), [AI 파이프라인](ai-pipeline.md), [전환 로드맵](../../planning/repository-separation-roadmap.md), [ADR-028](../11-decisions/ADR-028-provider-runtime-artifact-contract.md)

## 목적과 현재 상태

DohaMusic은 제품 서비스와 Pipeline Orchestration을 소유하고, 모델별 Dataset·학습·평가·추론 Runtime은 독립 AI Provider 저장소가 소유하는 방향을 채택한다. 저장소 책임 경계의 결정과 실제 Runtime 이전은 별도 단계다.

DohaLM은 별도 저장소로 존재한다. `DohaAudio`와 `DohaVocal`은 아직 생성되지 않은 `[계획]` 저장소이며, Runtime API·공통 Model Registry·Artifact URI 전환도 구현되지 않았다. 기존 DohaMusic의 ACE-Step·Demucs·Seed-VC Adapter와 subprocess Runner는 단계적 전환 기간의 호환 계층으로 유지한다.
...
flowchart LR
  ...
  M --> O[DohaMusic Pipeline Orchestrator]
  O --> L[DohaLM Provider]
...
- 모든 Workflow, Provider 선택, 작업 상태와 GPU admission control은 DohaMusic Pipeline Orchestrator가 관리한다.
...
- Provider 선택·호출과 Pipeline Orchestration
...
신규 Music Generator는 DohaMusic 내부에 추가하지 않고 DohaAudio에서 구현한다. DohaAudio 저장소와 Runtime API가 아직 없으므로 현재 기능으로 표시하지 않는다.
...
| `PipelineExecutor` | [완료] | DohaMusic에 유지 |
