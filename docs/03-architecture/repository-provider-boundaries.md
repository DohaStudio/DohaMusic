...
DohaLM, [DohaAudio](https://github.com/DohaStudio/DohaAudio)와 [DohaVocal](https://github.com/DohaStudio/DohaVocal)은 실제 저장소로 존재한다. DohaVocal은 single-process metadata-only Fake Runtime Foundation을 구현했고 DohaMusic은 JSON fixture 기반 Consumer Contract Foundation을 구현했다. Production transport·Artifact payload·실제 Vocal model·공통 Model Registry·Artifact URI 전환은 `[미구현]`이다. 기존 DohaMusic의 ACE-Step·Demucs·Seed-VC Adapter와 subprocess Runner는 단계적 전환 기간의 호환 계층으로 유지한다.
...
flowchart LR
  ...
  O --> V[DohaVocal 계약 기반·Production 연결 미구현]
...
신규 Singing Voice·Voice Conversion 구현은 DohaVocal에서 수행한다. 현재 DohaVocal은 실제 audio·AI 없이 4개 capability와 9개 operation의 metadata-only Fake Runtime을 제공하고 DohaMusic은 이를 strict consumer 계약으로 검증한다. 이는 Singing Voice·Voice Conversion model, Production transport 또는 운영 통합이 구현됐음을 의미하지 않는다.
