...
Workspace Job Worker는 이 ThreadPool 구현과 분리한다. `JobWorkerService.run_once()`의 `queued` atomic claim, claim token, Worker identity, lease·heartbeat·attempt, 내부 cancellation marker와 terminal protection은 구현됐다. concrete Provider Dispatcher, DohaVocal transport wiring과 background daemon은 구현되지 않았다. 현재 running lease 만료는 retryable failure로 종료한다. 목표 계약은 replay-safe Provider-backed Job의 graceful yield와 expired `running -> running` reclaim이며 구현 전에는 사용할 수 없다. Provider 성공 뒤 reconciliation은 [DohaVocal Worker Reconciliation Contract](dohavocal-worker-reconciliation-contract.md), ownership transfer는 [Worker Re-entry Lifecycle](workspace-worker-reentry-lifecycle.md), 실행 기반은 [Workspace Job Foundation](workspace-job-foundation.md)과 [ADR-033](../11-decisions/ADR-033-workspace-job-execution-boundary.md)을 따른다.
