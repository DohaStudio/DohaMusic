...
> 현재 상태: Legacy 공유 단일 ThreadPool Worker 구현 / Workspace claim·lease Worker 미구현
...
Workspace Job Worker는 이 ThreadPool 구현 완료 상태와 분리한다. 후속 기반은 `queued` atomic claim, claim token, Worker identity, lease·heartbeat·attempt, 내부 cancellation marker와 Provider idempotency를 요구한다. running lease 만료는 원본 row를 자동 재실행하지 않고 retryable failure로 종료하며 공개 retry는 새 Job을 만든다. 세부 계약은 [Workspace Job Foundation](workspace-job-foundation.md)과 [ADR-033](../11-decisions/ADR-033-workspace-job-execution-boundary.md)을 따른다.
