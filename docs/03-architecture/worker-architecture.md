...
> 현재 상태: 공유 단일 ThreadPool + 생성·Stem·Voice Provider-neutral Worker 구현
...
ACE-Step, Demucs, Seed-VC는 작업별 격리 subprocess를 사용하며 요청 중 모델을 자동 다운로드하지 않는다. 모든 AI dispatcher는 GPU 동시성 1인 executor를 공유한다. 외부 Queue·재시도·취소·다중 GPU는 후속 범위다.
