...
[ADR-040](../11-decisions/ADR-040-canonical-track-clip-working-composition-authority.md)은 mutable WorkingComposition과 canonical Track·Clip, exact AssetVersion과 불변 Snapshot commit 경계를 설계했다. ADR-045·047·050 authority와 schema·Repository·trusted duration·idempotency foundation, [atomic mutation Service](working-composition-service.md), 17개 Product operation, Frontend Track/Clip editing·memory Undo/Redo와 exact source-window Waveform을 구현했다. Composition commit과 working preview/render는 아직 구현하지 않았고 D1 snapshot-local projection은 계속 canonical Track이 아니다.
...
| 영역 | 현재 Gap |
|---|---|
| Composition edit model | ADR-040·ADR-050과 5개 persistence table·Service·17개 Product operation, Frontend UI·memory Undo/Redo 구현; Composition commit 미구현 |
...
| Frontend | Track/Clip basic editing·source-window Waveform 구현; working preview/render·Mixer·AI Director·QA page 미구현 |
...
- canonical Section identity와 편집 표현. canonical Track·Clip·WorkingComposition Backend, basic Timeline UI·memory Undo/Redo와 Track/Clip Waveform은 구현했지만 working preview/render·Composition commit은 미구현
