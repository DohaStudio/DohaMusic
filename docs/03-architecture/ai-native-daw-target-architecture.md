...
> 문서 상태: [운영 기준]
> 구현 상태: [D1·D2 Timeline Playback·Master/Mix Waveform CURRENT / D3 Clip Editing·Track/Clip Waveform 구현 / 장기 TARGET 부분 구현]
> 최종 수정일: 2026-08-28
...
현재 Frontend는 생성 Wizard와 결과 탐색, Project별 Composition read projection 및 WorkingComposition Track/Clip editing·memory Undo/Redo를 제공한다. ready 화면은 단일 Global Player 권위의 committed Timeline·Master/Mix Waveform과 별도로 canonical working Track/Clip과 exact source-window Waveform을 표시한다. working preview/render, Composition commit, 다중 Track Mixer, AI 후보 선택과 Composition QA 화면은 없다.
...
[ADR-040](../11-decisions/ADR-040-canonical-track-clip-working-composition-authority.md)은 mutable WorkingComposition과 canonical Track·Clip, exact AssetVersion과 불변 Snapshot commit 경계를 설계했다. ADR-045·047·050·052 authority와 schema·Repository·trusted duration·idempotency foundation, [atomic mutation Service](working-composition-service.md), 18개 Product operation, Frontend Track/Clip editing·memory Undo/Redo와 exact source-window Waveform 및 Working Preview Backend foundation을 구현했다. Preview Frontend와 Composition commit은 아직 구현하지 않았고 D1 snapshot-local projection은 계속 canonical Track이 아니다.
...
| 영역 | 현재 Gap |
|---|---|
| Composition edit model | ADR-040·ADR-050·ADR-052와 9개 persistence table·Service·18개 Product operation, Frontend UI·memory Undo/Redo·Preview Backend foundation 구현; Preview Frontend·Composition commit 미구현 |
...
| Frontend | Track/Clip basic editing·source-window Waveform 구현; Working Preview integration·Mixer·AI Director·QA page 미구현 |
...
- canonical Section identity와 편집 표현. canonical Track·Clip·WorkingComposition Backend, basic Timeline UI·memory Undo/Redo, Track/Clip Waveform과 Preview Backend foundation은 구현했지만 Preview Frontend·Composition commit은 미구현
