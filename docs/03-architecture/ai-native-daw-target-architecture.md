...
현재 Frontend는 생성 Wizard와 결과 탐색, Project별 Composition read projection 및 WorkingComposition Track/Clip basic editing·memory Undo/Redo를 제공한다. ready 화면은 단일 Global Player 권위의 committed Timeline·Master/Mix Waveform과 별도로 canonical working Track/Clip을 편집한다. Track/Clip 고도 Waveform, working preview/render, Composition commit, 다중 Track Mixer, AI 후보 선택과 Composition QA 화면은 없다.
...
[ADR-040](../11-decisions/ADR-040-canonical-track-clip-working-composition-authority.md)은 mutable WorkingComposition과 canonical Track·Clip, exact AssetVersion과 불변 Snapshot commit 경계를 설계했다. ADR-045·047·050 authority와 schema·Repository·trusted duration·idempotency foundation, [atomic mutation Service](working-composition-service.md), 17개 Product operation 및 Frontend Track/Clip editing·memory Undo/Redo를 구현했다. Composition commit은 아직 구현하지 않았고 D1 snapshot-local projection은 계속 canonical Track이 아니다.
...
| Frontend | Track/Clip basic editing 구현; Track/Clip Waveform·Mixer·AI Director·QA page 미구현 |
...
- canonical Section identity와 편집 표현. canonical Track·Clip·WorkingComposition Backend와 basic Timeline UI·memory Undo/Redo는 구현했지만 Track/Clip Waveform·Composition commit은 미구현
