...
## K3 비차단 후처리 계약 [구현 완료: K3.1·K3.2·K3.3]

K3.1·K3.2·K3.3은 다음 경계를 구현했다.
...
최종 WAV·기본 metadata·file row와 Pipeline `COMPLETED`를 먼저 확정하고 `PENDING` 분석 metadata를 둔다. 그 뒤 `DefaultAudioQualityAnalyzer`, `DefaultTempoAnalyzer`, `DefaultHookAnalyzer`가 final file role의 WAV를 독립적으로 읽어 기존 `audio_analysis` key만 병합한다. 요청 BPM은 오차와 half/double 후보 비교에만 쓰고 Tempo estimator를 유도하지 않는다. Hook은 에너지·반복 기반 추정 후보이며 Chorus 확정값이 아니다. 분석 예외·DB 갱신 오류는 Worker 실패 경계 밖에서 처리하므로 Job과 final WAV capability를 되돌리지 않는다. 새 Pipeline step·DB table·Migration·Provider 의존성은 없다. Preview는 K3.4 계획이다.
