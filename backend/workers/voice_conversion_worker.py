...
class VoiceConversionWorker:
    def __init__(self, session_factory: Callable[[], Session], voice_converter: VoiceConverter, storage: StorageService) -> None:
    ...
    def run(self, job_id: str) -> None:
        ...
        with self.session_factory() as session:
            ...
            try:
                ...
                logger.info("voice_inference_started job_id=%s model=%s", job_id, self.voice_converter.model_name)
                ...
                repository.add_file(job.id, "converted_voice", self.storage.relative_path(result.converted_path), "audio/wav")
                if result.metadata_path is not None:
                    repository.add_file(job.id, "metadata", self.storage.relative_path(result.metadata_path), "application/json")
                ...
                logger.info(
                    ...
                    job_id, result.provider, result.model_name, result.model_version,
                    round(result.conversion_time_seconds, 3), result.peak_vram_mb,
                )
                ...
                session.rollback()
                repository.mark_failed(job, "VOICE_CONVERSION_FAILED", "Voice Conversion 작업이 실패했습니다.")
            finally:
                logger.info("voice_worker_finished job_id=%s duration_ms=%s", job_id, round((time.perf_counter() - started_at) * 1000, 2))
