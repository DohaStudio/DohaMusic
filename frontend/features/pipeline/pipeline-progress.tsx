...
export function PipelineProgress({ jobId }: { jobId: string }) {
  ...
  useEffect(() => {
    patch({ pipelineJobId: jobId, currentStep: "generation" });
  }, [jobId, patch]);
  useEffect(() => {
  ...
  if (query.error && !job) {
    ...
    return (
      <div className="progress-page">
        <ErrorAlert
          title={
            missing ? "Job을 찾을 수 없습니다" : "Backend 연결이 불안정합니다"
          }
          message={
            missing
              ? "Job ID를 확인해 주세요. 자동 조회를 중단했습니다."
              : "Job은 취소되지 않았습니다. 같은 Job ID로 다시 조회합니다."
          }
        />
  ...
  return (
    <section className="progress-page">
      ...
      <p aria-live="polite" className="sr-status">
        현재 단계 {current.label}, {job.progress_percent}% 완료
      </p>
      <Progress
        value={job.progress_percent}
        label={`생성 진행률 ${job.progress_percent}%`}
      />
      {query.error && (
        <ErrorAlert
          title="Backend 연결이 불안정합니다"
          message="Job은 취소되지 않았습니다. 같은 Job ID로 다시 조회합니다."
        />
      )}
      <ol className="pipeline-stages">
        {stages.map((status, index) => {
          ...
          return (
            <li key={status} className={active ? "active" : done ? "done" : ""}>
              <span>{done ? "✓" : index + 1}</span>
              {mapPipelineStatus(status).label}
            </li>
          );
        })}
      </ol>
      {job.status === "FAILED" && (
        <ErrorAlert
          title={`Pipeline 실패 · ${job.failed_step ?? "단계 미상"}`}
          message={
            job.error_message ??
            job.error_code ??
            "안전한 오류 정보가 없습니다."
      ...
      <div className="actions">
        {job.status === "FAILED" && (
          ...
          >
            Review로 돌아가 새 Job 생성
          </Link>
        )}
        <Unsupported>Job 취소</Unsupported>
      </div>
    </section>
  );
}
