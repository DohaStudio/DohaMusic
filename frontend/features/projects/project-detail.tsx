...
export function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  return <section className="collection-page"><header className="collection-header"><div><p className="eyebrow">프로젝트</p><h1>{project.title}</h1><p>{project.description ?? "설명 없음"}</p></div><Link className="button secondary" href="/projects">목록</Link></header><ProjectEditor key={project.updated_at} projectId={projectId} title={project.title} description={project.description ?? ""} onSaved={() => query.refetch()} /><CompositionWorkspace projectId={projectId} /><div className="history-list">{project.jobs.length ? project.jobs.map((job) => { const analysis = parseAudioAnalysis(job.audio_analysis); return <article className="history-row" key={job.job_id}><div><h2>{job.title}</h2><p>{new Date(job.created_at).toLocaleString("ko-KR")} · {job.duration}초</p><GenerationOptionsSummary options={job.generation_options} retryOfJobId={job.retry_of_job_id} /><AudioQualitySummary compact analysis={analysis} /><TempoSummary mode="summary" analysis={analysis} /><HookSummary mode="summary" analysis={analysis} /></div><Badge tone={job.status === "COMPLETED" ? "success" : job.status === "FAILED" ? "error" : "active"}>{job.status === "COMPLETED" ? "완성" : job.status === "FAILED" ? "완료하지 못함" : job.status === "CANCELLED" ? "취소됨" : job.status === "CANCEL_REQUESTED" ? "취소 중" : "만드는 중"}</Badge><Link className="button" href={job.status === "COMPLETED" ? `/result/${job.job_id}` : `/generation/${job.job_id}`}>자세히 보기</Link></article>; }) : <div className="empty-state"><h2>아직 담긴 음악이 없습니다.</h2></div>}</div></section>;
}
