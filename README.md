...
Bootstrap 최신 Track: #164~#170/#172~#181은 merged다. [ADR-092 fresh journal-lineage observation](docs/11-decisions/ADR-092-fresh-journal-lineage-observation-foundation.md)은 별도 journal transaction의 complete history/head를 held lineage/pin/lease와 결합한다. [ADR-093 scoped provisioning authority](docs/11-decisions/ADR-093-scoped-provisioning-authority-verification-foundation.md)와 [ADR-094 authenticated authority source](docs/11-decisions/ADR-094-authenticated-scoped-provisioning-authority-source-foundation.md)는 root-signed complete lifecycle을 custody-bound fixed source와 결합한다. 새 [ADR-095 active verifier material](docs/11-decisions/ADR-095-active-provisioning-verifier-material-foundation.md)은 ACTIVE projection을 exact custody-bound Ed25519 public bytes와 연결한다. 결과는 signature/currentness witness/admission이 아니며 production ports는 unavailable, schema/migration과 Alembic 0037은 유지된다.
