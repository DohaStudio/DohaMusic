...
> F6 Guided Voice Enrollment는 `[계획]`입니다. Profile 1:N Sample·대표 reference, 임시 Enrollment의 ORM·Repository·상태 전이와 Alembic `0010` backfill은 구현했습니다. 브라우저 녹음·WAV 정규화·Enrollment API·Storage·cleanup worker·Wizard는 미구현입니다. Phase 8 완료 상태와 Phase 7 개인화 Dataset 학습을 변경하지 않습니다.
...
| [계획] | 사용자 안내형 Voice Enrollment Wizard, 브라우저 녹음·다중 sample·기본 품질 확인 |
| [완료] | Guided Voice Enrollment 영속 모델·상태 전이·기존 Profile 호환 backfill |
