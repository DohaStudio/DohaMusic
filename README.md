...
Bootstrap 최신 Track: #164~#170/#172~#183은 merged다. [ADR-092 fresh journal-lineage observation](docs/11-decisions/ADR-092-fresh-journal-lineage-observation-foundation.md)은 별도 journal transaction의 complete history/head를 held lineage/pin/lease와 결합한다. [ADR-093 scoped provisioning authority](docs/11-decisions/ADR-093-scoped-provisioning-authority-verification-foundation.md)~[ADR-096 Original Confirmation authenticity](docs/11-decisions/ADR-096-original-confirmation-authenticity-foundation.md)는 root-signed ACTIVE verifier material과 exact confirmation-domain signature를 검증한다. 새 [ADR-097 exact correlation](docs/11-decisions/ADR-097-authentic-confirmation-lineage-correlation-foundation.md)은 authenticity·held live lineage·fresh observation을 같은 handles/lease/두 transaction에 결합한다. 결과는 CurrentnessWitness/admission이 아니며 production ports는 unavailable, schema/migration과 Alembic 0037은 유지된다.
...
- [ADR-040](docs/11-decisions/ADR-040-canonical-track-clip-working-composition-authority.md)·[ADR-045](docs/11-decisions/ADR-045-clip-service-deletion-media-duration-authority.md)·[ADR-050](docs/11-decisions/ADR-050-working-composition-inverse-mutation-authority.md)·[ADR-052](docs/11-decisions/ADR-052-working-composition-preview-render-authority.md)·[ADR-053](docs/11-decisions/ADR-053-clip-gain-authority.md)·[ADR-054](docs/11-decisions/ADR-054-clip-fade-authority.md)·[ADR-055](docs/11-decisions/ADR-055-clip-timeline-duration-and-loop-phase-authority.md)에서 mutable WorkingComposition, canonical Track·Clip과 Loop geometry를 확정했다. Frontend Track/Clip 편집, source-window Waveform, Preview, Commit과 selected Clip Gain·Fade·Loop UI 및 reload-safe Backend persistent Undo/Redo를 구현했다.
...
- AI Music Director의 immutable Snapshot 기반 Job, Mock Provider Worker, durable Candidate proposal materialization, Project-scoped Run/Candidate read와 CAS SELECT Public API 기반. ADR-073 APPLY architecture는 승인됐고 실제 Provider, Frontend 비교 UX와 APPLY 구현은 미완료
...
- Section·Mixer·range selection과 persistent history
...
- 현재 제품 단계: D1 Composition Read Workspace [완료] / D2 Timeline Playback·Master/Mix Waveform·Richer Playhead [완료] / D3 WorkingComposition 기반 [진행 중] / D5 AI Music Director Backend Candidate 생성·read·SELECT Foundation [부분 구현] / Candidate APPLY architecture [승인, 구현 대기]
