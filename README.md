...
> Workspace Job Foundation: [공식 계약](docs/03-architecture/workspace-job-foundation.md)과 [ADR-033](docs/11-decisions/ADR-033-workspace-job-execution-boundary.md)에 따라 role·Workspace scope·cancel marker·claim/lease Column과 공개 keyset·Worker Index를 revision `20260810_0017`로 구현하고 실제 사용자 DB에 적용했습니다. `job` HMAC Cursor, Owner·Workspace scope Repository keyset page에 이어 Job 생성·입력 lineage·상태·진행률·취소 marker·재시도·멱등성·aggregate read를 Service transaction 경계로 구현했습니다. 기존 row 보존을 위해 scope와 role은 nullable staging이며 Worker claim/lease·Provider 호출·Completion Unit of Work·API는 미구현입니다. Job API는 0/5, Resource API는 25/64이고 Backend Foundation·Generative AI Track 완료 상태로 승격하지 않습니다.
