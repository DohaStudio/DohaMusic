...
| Workspace Artifact Domain | [진행 중] | Entity·Repository·Service, 실제 DB 0012·0013·0014와 첫 Resource API 11개 완료; 나머지 53개 API·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
12. [Workspace v1 API 계약](docs/06-api/workspace-rest-api-contract.md)은 공통 Router·응답 Schema·request ID·오류 분기, [명시적 Bootstrap 도구](docs/06-api/workspace-api-foundation-bootstrap.md), [HMAC Cursor Pagination](docs/06-api/cursor-pagination.md), 실제 적용된 Workspace·Project·ProjectAsset keyset Index와 Workspace·MusicProject·ProjectAsset Resource Endpoint 11개를 구현했다. ProjectAsset POST는 기존 Asset만 연결하고 같은 `(project_id, asset_id)` 관계를 복원하며, DELETE는 관계만 Soft Delete한다. 다음 범위는 Asset Cursor·Index와 Resource API 5개다. Resource API 진행도는 11/64다.
