...
> DohaVocal `0.2.0` consumer DTO·trust gate·transient payload acquisition foundation은 구현되었습니다. `DURABLE_LOCATOR_REQUIRED` 재분석은 verified staging·revocation·cleanup을 위한 전용 `PayloadLocator` persistence가 필요하다고 판정했습니다. 다음 Provider Track 범위는 domain model·persistence·SQLite·Alembic·issue/replay/revoke/resolve이며 durable staging·downloader·Artifact ingestion은 별도 계획입니다.
>
...
2. Phase B `[진행 중]`: DohaVocal `0.1.0` 호환과 `0.2.0` payload Result DTO·trust gate·transient acquisition, DohaMusic-owned Trusted Payload locator/issuer/resolver Foundation을 구현했고 Worker reconciliation·re-entry와 locator 전 handoff authority를 확정했다. [Durable Payload Locator Authority](docs/03-architecture/durable-payload-locator-authority.md)는 source 재구성에는 Result replay가 충분하지만 verified staging·revocation·cleanup에는 전용 aggregate가 필요하다고 판정했다. PayloadLocator schema/Runtime, reclaim runtime, concrete Vocal Worker wiring·인증·downloader·durable staging·Completion adapter·실제 model·Artifact payload 통합은 남아 있다.
...
DohaVocal `0.1.0` Fake Runtime 호환과 DohaMusic `0.2.0` Consumer DTO·capability negotiation·config 기반 transient binary HTTP acquisition·Result trust gate는 구현했다. dedicated PayloadLocator persistence는 architecture만 확정했으며 실제 schema/Runtime·Worker 연결·인증·durable staging·Artifact payload·AssetVersion commit, DohaAudio Runtime과 공통 Model Registry는 구현하지 않았다.
...
| Workspace Artifact·Job Domain | [진행 중] | Job Service·Completion UoW·Worker 실행 기반·공식 API 5/5, 4개 Vocal Job 계약, Provider Job 1:N persistence, `0.1.0`/`0.2.0` Result trust gate·transient acquisition과 Trusted Payload resolver Foundation 구현, dedicated locator authority 확정; Provider dispatch wiring·downloader·locator schema/Runtime·durable staging·Completion adapter·실제 payload ingestion·background daemon과 나머지 API 미구현 | [Workspace Job Foundation](docs/03-architecture/workspace-job-foundation.md) |
...
15. [DohaVocal Consumer Contract](docs/03-architecture/dohavocal-consumer-contract.md)은 4개 capability·version별 9/10 operation·`0.1.0` metadata-only와 `0.2.0` payload Result·acquisition을 검증한다. [Provider Result Ingestion Contract](docs/03-architecture/provider-result-ingestion-contract.md)은 두 version의 result를 검증하되 Artifact로 승격하지 않는다. [Durable Execution Handoff Analysis](docs/03-architecture/durable-execution-handoff-analysis.md)은 locator 전 새 execution storage가 불필요함을, [Durable Payload Locator Authority](docs/03-architecture/durable-payload-locator-authority.md)은 source replay와 별개로 verified staging·revocation·cleanup에 `DURABLE_LOCATOR_DEDICATED_AUTHORITY_REQUIRED`임을 확정한다. 다음 Provider Track 구현은 PayloadLocator domain·persistence·SQLite·Alembic·issue/replay/revoke/resolve까지만이며 durable staging·downloader·Completion·Artifact ingestion은 별도 후속이다.
