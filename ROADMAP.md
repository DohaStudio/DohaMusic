...
17. [부분 완료] D2 Timeline Playback Foundation은 기존 Global Player를 단일 audio authority로 재사용한다. 선택된 Snapshot의 단일 `mix` Item·단일 safe audio Artifact만 source로 해석하고, 실제 media metadata의 duration·currentTime으로 초 단위 ruler와 Playhead를 구동한다. 모호한 source는 `NO_CANONICAL_PLAYBACK_SOURCE`로 비활성화하며 Waveform·Clip·Section·Mixer·range selection은 미구현이다.
