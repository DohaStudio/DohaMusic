...
3. D3 `[진행 중]`: ADR-040·045·047·050·052의 WorkingComposition/Preview authority, 9개 persistence table, atomic mutation Service와 20개 Product operation, Frontend Track/Clip editing·explicit Clip Copy·memory Undo/Redo, exact AssetVersion-safe media source·Clip별 source-window Waveform, Working Preview와 Composition Commit Backend/Frontend integration을 구현했다. D4 Mixer·독립 Export는 `[계획]`이다.
...
Clip Persistence·Authority, revision-safe idempotency와 WorkingComposition atomic mutation Service·20개 Product operation을 구현했다. Frontend는 exact AssetVersion Clip create와 explicit destination Copy, Track/Clip mutation, response revision 기반 reconcile, same-ID restore·unsplit/resplit memory Undo/Redo, Clip별 `[source_in, source_out)` Waveform, Working Preview와 Composition Commit을 소비한다. Backend는 exact AssetVersion lineage와 canonical geometry를 새 immutable Snapshot으로 고정하고 revision-pinned Working Preview Job은 non-canonical Preview Asset/AssetVersion/Artifact로 완성한다. Provider·Training·Dataset·GPU·Common Contract는 변경하지 않았고 Fade·Gain·Loop·Section·Mixer·range selection은 구현하지 않았다. Phase 8 `100%`는 로컬 MVP 판정이며 이 Track의 완료율이 아니다.
...
| AI-native DAW Product | [진행 중] | D0·D1·D2와 D3 WorkingComposition·Preview·Composition Commit Backend/Frontend integration, Clip editing·explicit Clip Copy·memory Undo/Redo·safe media source·Track/Clip Waveform 완료; Fade·Gain·Loop·실제 DB 전환·Section·Mixer·D4~D9 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
**최우선 NEXT:** Clip Copy 다음으로 Fade·Gain·Loop 중 authority 선행이 가능한 단일 D3 editing gate를 진행한다. 이후 AI Segment Editing 순서로 확장한다. MIDI·Piano Roll은 NOT IMPLEMENTED, SoundFont는 NOT INTEGRATED 상태의 별도 우선순위다. 실제 사용자 DB `0017 → 0024` 전환은 별도 승인 Gate로 유지하며, Workspace Job background daemon activation은 별도 Track이다.
...
16. [진행 중] AI-native DAW D1-A~D2와 D3 WorkingComposition persistence·Service·20개 Product API, Frontend Track/Clip UI·explicit Clip Copy·memory Undo/Redo, exact AssetVersion-safe media source·Track/Clip Waveform, Working Preview와 Composition Commit Backend/Frontend integration을 완료했다. expected revision CAS, revision-safe replay와 revision-pinned manifest를 소비하지만 canonical Section은 미구현이며 신규 `EditIntent`를 만들지 않았다.
