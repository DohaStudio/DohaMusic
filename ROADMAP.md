...
Bootstrap 최신 실행 순서: #164 ADR-078 Contract merged → 별도 Draft [ADR-079 Journal Foundation](docs/11-decisions/ADR-079-independent-lifecycle-journal-persistence-foundation.md) 구현·검증 → independent provisioning/private admission/currentness·OS ceremony serialization·pin install/reconciliation → Installation/Claim·principal/binding/Auth → Rights Writer/Adapter/Runtime. public persistence는 운영 권한이 아니며 기존 Phase/DoD 진행률은 유지한다. 아래 직전 Contract 단계 기록의 NEXT는 이 실행 순서로 갱신한다.
