...
| Workspace Artifact Domain | [진행 중] | Entity·실제 DB additive migration·Repository·Service 완료, API 공통 기반·명시적 Bootstrap 구현 중, Resource API·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
12. [Workspace v1 API 계약](docs/06-api/workspace-rest-api-contract.md)은 Common Specification과 DB Redesign 기준으로 공통 Router·응답 Schema·request ID·오류 분기와 [명시적 Bootstrap 도구](docs/06-api/workspace-api-foundation-bootstrap.md)를 구현 중이다. Resource Endpoint 64개, Idempotency replay, cursor codec과 Frontend는 아직 구현하지 않는다.
