...
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 21개 SQLAlchemy 2.0 Entity, additive revision `20260806_0012`, read-only Preflight·backup 도구와 fixture rehearsal까지 `[진행 중]`이다. 실제 사용자 DB 접근·적용·backfill·dual write·Repository·Service·API는 미구현이며 현재 14개 Runtime Table과 source of truth는 변경하지 않는다. 실제 적용은 [Runbook](docs/10-operations/workspace-db-migration-runbook.md)의 BLOCKER 해소와 사용자 승인이 필요하다.
