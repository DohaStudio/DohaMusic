...
Bootstrap 최신 실행 순서: #164/#165/#166/#167 [ADR-081 witness lifetime helper](docs/11-decisions/ADR-081-provider-witness-lifetime-foundation.md) merged → 별도 Draft [ADR-082 Windows serialization mechanics](docs/11-decisions/ADR-082-windows-ceremony-serialization-foundation.md) → reviewed independent private evidence/pin/journal/provenance readers·complete manifest/currentness verification → explicit durable admission owner/same-event pin install·reconciliation → Installation/Claim·principal/binding/Auth → Rights Writer/Adapter/Runtime. OS acquisition/public scope/helper lifetime check는 authority가 아니며 production composition unavailable 및 기존 Phase/DoD 진행률은 유지한다. 아래 직전 단계 기록의 NEXT는 이 실행 순서로 갱신한다.
