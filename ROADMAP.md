...
2. Phase B `[진행 중]`: DohaVocal Fake Runtime과 DohaMusic Consumer Contract·HTTP Transport·metadata-only Result trust gate, DohaMusic-owned Trusted Payload locator/issuer/resolver Foundation을 구현했다. 신규 Music Generator와 Vocal Worker wiring·인증·downloader·Completion adapter·실제 model·Artifact payload 통합은 남아 있다.
...
DohaVocal `0.1.0` Fake Runtime과 DohaMusic Consumer DTO·mapping·config 기반 HTTP transport·Mock HTTP contract test, metadata-only Result의 binding·role·Manifest·lineage·checksum scope trust gate는 구현했다. 실제 Worker 연결, 인증, Artifact payload·AssetVersion commit, DohaAudio Runtime과 공통 Model Registry는 구현하지 않았다.

DohaVocal은 metadata-only Runtime API와 DohaMusic HTTP adapter 기반을 제공한다. DohaAudio Runtime API, 실제 Artifact URI·payload와 공통 Model Registry는 구현하지 않았다.
...
3. D3 `[설계 완료 / 구현 NEXT]`: [ADR-040](docs/11-decisions/ADR-040-canonical-track-clip-working-composition-authority.md)에서 mutable WorkingComposition, canonical Track·Clip, exact AssetVersion과 불변 Snapshot commit 경계를 확정했다. schema·API·Clip UI는 미구현이다. D4 Mixer·독립 Export는 `[계획]`이다.
...
이번 Clip Domain / Persistence 작업은 문서 설계만 변경한다. Backend·Frontend source, 테스트, Alembic·API·DB·Provider·Training·Dataset·GPU·Common Contract는 변경하지 않는다. 편집 가능한 Track/Clip Waveform·Clip UI·Section·Mixer·range selection은 구현하지 않았다. Phase 8 `100%`는 로컬 MVP 판정이며 이 Track의 완료율이 아니다.
...
| AI-native DAW Product | [진행 중] | D0·D1·D2와 Clip Domain/Persistence 설계 완료; WorkingComposition·Track·Clip schema/API/UI, 실제 DB 전환·Section·Mixer·D3 구현·D4~D9 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
| Workspace Artifact·Job Domain | [진행 중] | Job Service·Completion UoW·Worker 실행 기반·공식 API 5/5, 4개 Vocal Job 계약, Provider Job 1:N persistence, metadata-only Result trust gate와 Trusted Payload resolver Foundation 구현; Provider dispatch wiring·downloader·Completion adapter·실제 payload ingestion·background daemon과 나머지 34개 API 미구현 | [Workspace Job Foundation](docs/03-architecture/workspace-job-foundation.md) |
...
| Reviewer Authentication Authority | [결정 완료 / 구현 미착수] | V1 `LOCAL_ONLY`, single owner/operator, DohaMusic local governance UI·OS-bound proof와 delegated assertion model 승인; DohaAudio Runtime selection·adapter·mapping 미구현 | [Authority](docs/09-security/reviewer-authentication-deployment-authority.md) |

## 현재 우선 작업

**최우선 NEXT:** ADR-040을 기반으로 Clip Editing Implementation을 진행한다. 순서는 Snapshot schema extension·source duration authority, WorkingComposition/Track/Clip persistence, Service/API, Frontend editing이며 이후 Track/Clip Waveform, working preview/render, Mixer, AI Segment Editing으로 확장한다. MIDI·Piano Roll은 NOT IMPLEMENTED, SoundFont는 NOT INTEGRATED 상태의 별도 우선순위다. 실제 사용자 DB `0017 → 0018` 전환은 별도 승인 Gate로 유지하며, Workspace Job의 Provider dispatch wiring과 background daemon은 별도 Track이다.
...
12. [Workspace v1 API 계약](docs/06-api/workspace-rest-api-contract.md)은 Resource Endpoint 30개와 D1 product API 2개를 구현했다. Bootstrap CLI는 source `20260821_0019`의 D1 Table·PK·unique·same-Project FK·identity Index와 transition inventory를 검증하며 실제 사용자 DB는 `20260810_0017`로 유지하고 실제 Bootstrap은 실행하지 않았다. Job API는 5/5이며 Resource API 진행도는 30/64다.
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 source Workspace Entity 23개, 별도 `ArtifactStorageLocation`, Workspace Repository와 [Service 소유 transaction](docs/03-architecture/workspace-service-transaction.md)을 구현했다. `20260820_0018`은 Project Composition selection, `20260821_0019`는 Provider Job 1:N identity·retry history를 additive하게 추가한다. source metadata는 38개 Table이고 실제 사용자 DB는 `20260810_0017`의 36개 Table이다. 격리 Bootstrap transition은 선택 authority가 없으므로 backfill 0건을 보장한다. 실제 Bootstrap·DB migration·data backfill·dual write·Provider dispatch wiring·background daemon·나머지 34개 Resource REST API·Frontend·Legacy 제거는 미구현이고 현재 14개 Runtime Table과 source of truth는 변경하지 않는다.
...
15. [DohaVocal Consumer Contract](docs/03-architecture/dohavocal-consumer-contract.md)은 4개 capability·9개 operation·Job·Retry·idempotency·lineage·Manifest·오류를 fake transport와 Mock HTTP로 검증한다. [Provider Result Ingestion Contract](docs/03-architecture/provider-result-ingestion-contract.md)은 metadata-only result를 검증하되 Artifact로 승격하지 않고, [Trusted Payload Locator / Resolver Contract](docs/03-architecture/trusted-payload-locator-resolver-contract.md)은 DohaMusic staging payload의 opaque locator·expiry·immutability·checksum·media 경계를 검증한다. 다음은 production downloader·Completion adapter와 Workspace Worker 조립이며 기존 subprocess Runner는 단계적 이전 전까지 유지한다.
16. [부분 완료] AI-native DAW D1-A~D2의 Composition read·Timeline·Waveform을 완료했다. ADR-040은 Project당 하나의 mutable WorkingComposition, canonical Track·Clip과 별도 불변 Snapshot Track/Clip commit 경계를 확정했다. schema·API·UI와 canonical Section은 미구현이며 신규 `EditIntent`를 만들지 않는다.
17. [결정 완료 / 구현 미착수] [ADR-037](docs/11-decisions/ADR-037-reviewer-authentication-deployment-authority.md)의 historical no-selection 경계를 유지하고 [ADR-038](docs/11-decisions/ADR-038-v1-reviewer-authentication-product-decision.md)에서 explicit product-owner authority를 반영했다. V1은 local-only·no general product login이며 single owner/operator가 DohaMusic local governance UI에서 OS-bound credential proof를 거쳐 review를 시작한다. DohaAudio adapter model은 `DOHAMUSIC_DELEGATED_ASSERTION`으로 선택 준비가 완료됐지만 Runtime selection·구현·mapping·authority·approval은 0이다.
