...
| Workspace Artifact Domain | [진행 중] | Entity·Repository·Service, 실제 DB 0012·0013·0014, Asset Cursor·source 0015와 첫 Resource API 11개 완료; 실제 DB 0015·나머지 53개 API·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
12. [Workspace v1 API 계약](docs/06-api/workspace-rest-api-contract.md)은 공통 Router·응답 Schema·request ID·오류 분기, [명시적 Bootstrap 도구](docs/06-api/workspace-api-foundation-bootstrap.md), [HMAC Cursor Pagination](docs/06-api/cursor-pagination.md), 실제 적용된 Workspace·Project·ProjectAsset keyset Index와 Workspace·MusicProject·ProjectAsset Resource Endpoint 11개를 구현했다. Asset은 현재 Owner 전체와 선택적 `workspace_id`·`asset_type` 공개 filter, `(created_at DESC, asset_id DESC)` Cursor·keyset Repository/Service와 source Index revision `20260808_0015`까지 구현했지만 Router 5개와 실제 DB 0015 적용은 미구현이다. Resource API 진행도는 11/64다.
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 21개 SQLAlchemy 2.0 Entity, additive revision `20260806_0012`, keyset Index revision `20260807_0013`과 ProjectAsset partial keyset Index revision `20260807_0014`의 실제 사용자 DB 적용, Asset full keyset Index source revision `20260808_0015`, Workspace Repository와 [Service 소유 transaction](docs/03-architecture/workspace-service-transaction.md)까지 완료했다. 실제 사용자 DB는 여전히 0014이며 실제 Bootstrap을 실행하지 않아 신규 Workspace Table 21개는 비어 있다. backfill·dual write·나머지 Resource REST API·Frontend·Legacy 제거는 미구현이고 현재 14개 Runtime Table과 source of truth는 변경하지 않는다.
