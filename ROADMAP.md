...
| Workspace Artifact Domain | [진행 중] | Entity·Repository·Service, 실제 DB 0012~0015와 Workspace·Project·ProjectAsset·Asset Resource API 16개 완료; 나머지 48개 API·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
12. [Workspace v1 API 계약](docs/06-api/workspace-rest-api-contract.md)은 공통 Router·응답 Schema·request ID·오류 분기, [명시적 Bootstrap 도구](docs/06-api/workspace-api-foundation-bootstrap.md), [HMAC Cursor Pagination](docs/06-api/cursor-pagination.md)과 Workspace·MusicProject·ProjectAsset·Asset Resource Endpoint 16개를 구현했다. Asset 목록은 Bootstrap Workspace에서 Owner를 파생하고 선택적 `workspace_id`·`asset_type`, `(created_at DESC, asset_id DESC)` keyset을 사용한다. Resource API 진행도는 16/64다.
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 21개 SQLAlchemy 2.0 Entity, additive revision `20260806_0012`와 keyset Index revision `20260807_0013`~`20260808_0015`의 실제 사용자 DB 적용, Workspace Repository와 [Service 소유 transaction](docs/03-architecture/workspace-service-transaction.md)까지 완료했다. 실제 Bootstrap을 실행하지 않아 신규 Workspace Table 21개는 비어 있다. backfill·dual write·나머지 48개 Resource REST API·Frontend·Legacy 제거는 미구현이고 현재 14개 Runtime Table과 source of truth는 변경하지 않는다.
