...
> DohaVocal `0.2.0` consumer, 전용 `PayloadLocator`, verified durable local staging과 payload acquisition orchestration을 구현했습니다. trusted candidate의 fixed-origin `GetPayloadContent`부터 `verified_staged` CAS까지 연결했으며 Artifact ingestion은 별도 후속입니다.
>
...
2. Phase B `[진행 중]`: DohaVocal `0.1.0` 호환과 `0.2.0` payload Result DTO·trust gate·transient acquisition, durable `PayloadLocator`, verified local staging과 acquisition orchestration을 구현했다. `source_bound → verified_staged`까지 실제 연결하며 terminal revocation, restart/idempotency/CAS를 보존한다. reclaim runtime, concrete Vocal Worker wiring·production 인증·Completion adapter·실제 model·Artifact ingestion은 남아 있다.
...
DohaVocal `0.1.0` Fake Runtime 호환과 DohaMusic `0.2.0` Consumer DTO·capability negotiation·config 기반 transient binary HTTP acquisition·Result trust gate, dedicated PayloadLocator, verified local staging과 acquisition orchestration은 구현했다. Worker 연결·production 인증·Artifact payload·AssetVersion commit, DohaAudio Runtime과 공통 Model Registry는 구현하지 않았다.
...
3. D3 `[진행 중]`: ADR-040·045·047·050의 WorkingComposition authority, 5개 persistence table, atomic mutation Service와 17개 Product operation에 이어 Frontend Track/Clip editing과 memory Undo/Redo를 구현했다. Composition commit과 Track/Clip 고도 Waveform은 미구현이며 D4 Mixer·독립 Export는 `[계획]`이다.
...
Clip Persistence·Authority, revision-safe idempotency와 WorkingComposition atomic mutation Service·17개 Product operation을 구현했다. Frontend는 exact AssetVersion Clip create, Track/Clip mutation, response revision 기반 reconcile, same-ID restore·unsplit/resplit memory Undo/Redo를 소비한다. Provider·Training·Dataset·GPU·Common Contract는 변경하지 않았고 Track/Clip 고도 Waveform·working preview/render·Composition commit·Section·Mixer·range selection은 구현하지 않았다. Phase 8 `100%`는 로컬 MVP 판정이며 이 Track의 완료율이 아니다.
...
| AI-native DAW Product | [진행 중] | D0·D1·D2와 D3 WorkingComposition Backend·Frontend Clip editing·memory Undo/Redo 완료; Track/Clip Waveform·Composition commit·실제 DB 전환·Section·Mixer·D4~D9 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
| Workspace Artifact·Job Domain | [진행 중] | Job Service·Completion UoW·Worker 실행 기반·공식 API 5/5, 4개 Vocal Job 계약, Provider Job 1:N, PayloadLocator·verified staging과 `0.1.0`/`0.2.0` Result trust gate·payload acquisition orchestration 구현; Provider dispatch·Worker wiring·Completion adapter·Artifact ingestion·background daemon과 나머지 API 미구현 | [Payload Acquisition Orchestration](docs/03-architecture/dohavocal-payload-acquisition-orchestration.md) |
...
**최우선 NEXT:** Frontend Clip Editing + memory Undo/Redo 다음으로 Track/Clip Waveform을 진행한다. 이후 working preview/render, Composition commit, Mixer, AI Segment Editing 순서로 확장한다. MIDI·Piano Roll은 NOT IMPLEMENTED, SoundFont는 NOT INTEGRATED 상태의 별도 우선순위다. 실제 사용자 DB `0017 → 0023` 전환은 별도 승인 Gate로 유지하며, Workspace Job의 Provider dispatch wiring과 background daemon은 별도 Track이다.
...
12. [Workspace v1 API 계약](docs/06-api/workspace-rest-api-contract.md)은 Resource Endpoint 30개와 D1 product API 2개를 구현했다. Bootstrap CLI는 source `20260825_0023`의 schema와 transition inventory를 검증하며 실제 사용자 DB는 `20260810_0017`로 유지하고 실제 Bootstrap은 실행하지 않았다. Job API는 5/5이며 Resource API 진행도는 30/64다.
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 source Workspace Entity 29개, 별도 `ArtifactStorageLocation`, Workspace Repository와 [Service 소유 transaction](docs/03-architecture/workspace-service-transaction.md)을 구현했다. `20260825_0023`은 durable PayloadLocator table과 binding scope index를 additive하게 추가한다. source metadata는 44개 Table이고 실제 사용자 DB는 `20260810_0017`의 36개 Table이다. 실제 Bootstrap·DB migration·data backfill·dual write·Provider dispatch wiring·background daemon·나머지 34개 Resource REST API·Frontend·Legacy 제거는 미구현이고 현재 14개 Runtime Table과 source of truth는 변경하지 않는다.
...
15. [DohaVocal Consumer Contract](docs/03-architecture/dohavocal-consumer-contract.md)은 4개 capability·version별 9/10 operation·`0.1.0` metadata-only와 `0.2.0` payload Result·acquisition을 검증한다. durable locator와 verified local staging 위에 [Payload Acquisition Orchestration](docs/03-architecture/dohavocal-payload-acquisition-orchestration.md)을 구현해 `GetPayloadContent → verified_staged`를 연결했다. 다음 순서는 Artifact ingestion과 Completion adapter이며 Worker/dispatcher wiring은 그 이후다.
16. [진행 중] AI-native DAW D1-A~D2와 D3 WorkingComposition persistence·Service·Product API, Frontend Track/Clip UI와 memory Undo/Redo를 완료했다. expected revision CAS, revision-safe replay와 reconcile을 소비하지만 Composition commit·canonical Section은 미구현이며 신규 `EditIntent`를 만들지 않았다.
