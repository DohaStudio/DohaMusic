...
| Workspace Artifact Domain | [진행 중] | Entity·Repository·Service, 실제 DB 0012·0013·0014, ProjectAsset Cursor 기반과 첫 Resource API 8개 완료; ProjectAsset API·나머지 56개 API·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
12. [Workspace v1 API 계약](docs/06-api/workspace-rest-api-contract.md)은 공통 Router·응답 Schema·request ID·오류 분기, [명시적 Bootstrap 도구](docs/06-api/workspace-api-foundation-bootstrap.md), [HMAC Cursor Pagination](docs/06-api/cursor-pagination.md), 실제 적용된 [Workspace·Project keyset 복합 Index](docs/07-database/workspace-keyset-indexes.md)와 Workspace·MusicProject Resource Endpoint 8개를 구현했다. ProjectAsset은 `display_order ASC, project_asset_id ASC` Cursor·Repository·Service page와 revision `20260807_0014`의 실제 사용자 DB 적용까지 완료했지만 Router 3개는 아직 계획이다. 다음 범위는 ProjectAsset Resource API 3개이며 이후 Asset Cursor·Index와 Resource API 5개를 진행한다. Resource API 진행도는 8/64를 유지한다.
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 21개 SQLAlchemy 2.0 Entity, additive revision `20260806_0012`, keyset Index revision `20260807_0013`과 ProjectAsset partial keyset Index revision `20260807_0014`의 실제 사용자 DB 적용, Workspace Repository와 [Service 소유 transaction](docs/03-architecture/workspace-service-transaction.md)까지 완료했다. 실제 Bootstrap은 실행하지 않아 신규 Workspace Table 21개는 비어 있으며 backfill·dual write·나머지 Resource REST API·Frontend·Legacy 제거는 미구현이다. 현재 14개 Runtime Table과 source of truth는 변경하지 않는다.
