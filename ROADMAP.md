...
Bootstrap 최신 실행 순서: #164 ADR-078 Contract / #165 [ADR-079 Journal Foundation](docs/11-decisions/ADR-079-independent-lifecycle-journal-persistence-foundation.md) merged → 별도 Draft D [ADR-080 private handoff Contract](docs/11-decisions/ADR-080-private-admission-currentness-handoff-contract.md) → A independent provisioning/private admission/currentness infrastructure·OS ceremony serialization·pin install/reconciliation → Installation/Claim·principal/binding/Auth → Rights Writer/Adapter/Runtime. public persistence는 운영 권한이 아니며 기존 Phase/DoD 진행률은 유지한다. 아래 직전 Contract 단계 기록의 NEXT는 이 실행 순서로 갱신한다.
