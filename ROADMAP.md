...
3. D3 `[진행 중]`: ADR-040·045·047·050의 WorkingComposition authority, 5개 persistence table, atomic mutation Service와 17개 Product operation, Frontend Track/Clip editing·memory Undo/Redo, exact AssetVersion-safe media source와 Clip별 source-window Waveform을 구현했다. working preview/render와 Composition commit은 미구현이며 D4 Mixer·독립 Export는 `[계획]`이다.
...
| AI-native DAW Product | [진행 중] | D0·D1·D2와 D3 WorkingComposition Backend·Frontend Clip editing·memory Undo/Redo·safe media source·Track/Clip Waveform 완료; working preview/render·Composition commit·실제 DB 전환·Section·Mixer·D4~D9 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
**최우선 NEXT:** Track/Clip Waveform Frontend 다음으로 working preview/render를 진행한다. 이후 Composition commit, Mixer, AI Segment Editing 순서로 확장한다. MIDI·Piano Roll은 NOT IMPLEMENTED, SoundFont는 NOT INTEGRATED 상태의 별도 우선순위다. 실제 사용자 DB `0017 → 0023` 전환은 별도 승인 Gate로 유지하며, Workspace Job의 Provider dispatch wiring과 background daemon은 별도 Track이다.
...
16. [진행 중] AI-native DAW D1-A~D2와 D3 WorkingComposition persistence·Service·Product API, Frontend Track/Clip UI·memory Undo/Redo, exact AssetVersion-safe media source와 Track/Clip Waveform을 완료했다. expected revision CAS, revision-safe replay와 reconcile을 소비하지만 working preview/render·Composition commit·canonical Section은 미구현이며 신규 `EditIntent`를 만들지 않았다.
