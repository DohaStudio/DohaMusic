...
Bootstrap 최신 실행 순서: #164~#169 [ADR-083 private pin facts transport/comparison](docs/11-decisions/ADR-083-private-pin-facts-reader-foundation.md) merged → 별도 Draft [ADR-084 designation provenance reader 입력·snapshot Contract](docs/11-decisions/ADR-084-designation-provenance-reader-input-contract.md) → reviewed independent private custody/designation/provenance infrastructure → fresh journal/history·authoritative complete manifest/possession/currentness verification → explicit durable admission owner/same-event pin install·reconciliation → Installation/Claim·principal/binding/Auth → Rights Writer/Adapter/Runtime. private file facts/서명/OS lock/helper는 authority가 아니며 production unavailable 및 Phase/DoD 진행률은 유지한다. 아래 직전 단계 기록의 NEXT는 이 실행 순서로 갱신한다.
