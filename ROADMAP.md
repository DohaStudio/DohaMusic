...
| Workspace Artifact Domain | [진행 중] | Entity·additive migration·DB preflight 도구 구현, 실제 DB·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 21개 SQLAlchemy 2.0 Entity, additive revision `20260806_0012`, read-only Preflight·backup 도구, startup 자동 Migration 기본 비활성화와 Runtime·Alembic SQLite FK 활성화까지 `[진행 중]`이다. 실제 사용자 DB 접근·Inventory·backup·적용·backfill·dual write·Repository·Service·API는 미구현이며 현재 14개 Runtime Table과 source of truth는 변경하지 않는다. 실제 적용은 [Runbook](docs/10-operations/workspace-db-migration-runbook.md)의 남은 BLOCKER 해소와 사용자 승인이 필요하다.
