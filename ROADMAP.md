...
3. D3 `[진행 중]`: ADR-040·045·047·050·052·053의 WorkingComposition/Preview/Clip Gain authority, 9개 persistence table, atomic mutation Service와 21개 Product operation, Frontend Track/Clip editing·explicit Clip Copy·memory Undo/Redo, exact AssetVersion-safe media source·Clip별 source-window Waveform, Working Preview와 Composition Commit Backend/Frontend integration, Clip Gain Backend foundation을 구현했다. D4 Mixer·독립 Export는 `[계획]`이다.
...
Clip Persistence·Authority, revision-safe idempotency와 WorkingComposition atomic mutation Service·21개 Product operation을 구현했다. Frontend는 exact AssetVersion Clip create와 explicit destination Copy, Track/Clip mutation, response revision 기반 reconcile, same-ID restore·unsplit/resplit memory Undo/Redo, Clip별 `[source_in, source_out)` Waveform, Working Preview와 Composition Commit을 소비한다. Backend는 exact AssetVersion lineage·canonical geometry·Clip Gain을 immutable Snapshot과 revision-pinned Working Preview manifest에 고정하고 Preview renderer에 static dB gain을 적용한다. Provider·Training·Dataset·GPU·Common Contract는 변경하지 않았고 Clip Gain Frontend UI·Fade·Loop·Section·Mixer·range selection은 구현하지 않았다. Phase 8 `100%`는 로컬 MVP 판정이며 이 Track의 완료율이 아니다.
...
| AI-native DAW Product | [진행 중] | D0·D1·D2와 D3 WorkingComposition·Preview·Composition Commit Backend/Frontend integration, Clip editing·explicit Clip Copy·memory Undo/Redo·safe media source·Track/Clip Waveform·Clip Gain Backend foundation 완료; Gain UI·Fade·Loop·실제 DB 전환·Section·Mixer·D4~D9 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
**최우선 NEXT:** Clip Gain Frontend UI 또는 Fade·Loop 중 authority 선행이 가능한 단일 D3 editing gate를 진행한다. 이후 AI Segment Editing 순서로 확장한다. MIDI·Piano Roll은 NOT IMPLEMENTED, SoundFont는 NOT INTEGRATED 상태의 별도 우선순위다. 실제 사용자 DB `0017 → 0025` 전환은 별도 승인 Gate로 유지하며, Workspace Job background daemon activation은 별도 Track이다.
...
12. [Workspace v1 API 계약](docs/06-api/workspace-rest-api-contract.md)은 Resource Endpoint 30개와 D1 product API 2개를 구현했다. Bootstrap CLI는 source `20260830_0025`의 schema와 transition inventory를 검증하며 실제 사용자 DB는 `20260810_0017`로 유지하고 실제 Bootstrap은 실행하지 않았다. Job API는 5/5이며 Resource API 진행도는 30/64다.
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 source Workspace Entity 33개, 별도 `ArtifactStorageLocation`, Workspace Repository와 [Service 소유 transaction](docs/03-architecture/workspace-service-transaction.md)을 구현했다. `20260825_0023`은 durable PayloadLocator를, `20260828_0024`는 Working Preview persistence 4개 table을, `20260830_0025`는 세 Clip table의 static Gain을 additive하게 추가한다. source metadata는 48개 Table이고 실제 사용자 DB는 `20260810_0017`의 36개 Table이다. 실제 Bootstrap·DB migration·data backfill·dual write·Provider dispatch wiring·background daemon·나머지 34개 Resource REST API·Frontend·Legacy 제거는 미구현이고 현재 14개 Runtime Table과 source of truth는 변경하지 않는다.
...
16. [진행 중] AI-native DAW D1-A~D2와 D3 WorkingComposition persistence·Service·21개 Product API, Frontend Track/Clip UI·explicit Clip Copy·memory Undo/Redo, exact AssetVersion-safe media source·Track/Clip Waveform, Working Preview와 Composition Commit Backend/Frontend integration, Clip Gain Backend foundation을 완료했다. expected revision CAS, revision-safe replay와 revision-pinned manifest를 소비하지만 Clip Gain Frontend UI와 canonical Section은 미구현이며 신규 `EditIntent`를 만들지 않았다.
