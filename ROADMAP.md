...
| AI-native DAW Product | [진행 중] | D0·D1·D2와 D3 WorkingComposition·Preview·Composition Commit·Clip Gain/Fade Backend/Frontend integration, Clip editing·explicit Clip Copy·memory Undo/Redo·safe media source·Track/Clip Waveform 완료; Loop·persistent history·multi-user recovery·실제 DB 전환·Section·Mixer·D4~D9 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
**최우선 NEXT:** ADR-055가 확정한 `timeline_duration + loop_enabled + loop_phase` geometry에 따라 Loop Authority / Foundation 구현 단일 D3 gate를 진행한다. MIDI·Piano Roll은 NOT IMPLEMENTED, SoundFont는 NOT INTEGRATED 상태의 별도 우선순위다. 실제 사용자 DB `0017 → 0026` 전환은 별도 승인 Gate로 유지하며, Workspace Job background daemon activation은 별도 Track이다.
...
16. [진행 중] AI-native DAW D1-A~D2와 D3 WorkingComposition persistence·Service·22개 Product API, Frontend Track/Clip UI·explicit Clip Copy·memory Undo/Redo, exact AssetVersion-safe media source·Track/Clip Waveform, Working Preview·Composition Commit과 Clip Gain/Fade Backend/Frontend integration을 완료했다. expected revision CAS, revision-safe replay와 revision-pinned manifest를 소비하지만 Loop·persistent history·multi-user recovery와 canonical Section은 미구현이며 신규 `EditIntent`를 만들지 않았다.
