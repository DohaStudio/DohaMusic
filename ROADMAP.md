...
2. D1 `[완료]`, D2 `[Foundation 구현·Draft 검토]`: Composition Read의 Workspace 권위와 Project 상세 연결을 완료했다. 선택된 Snapshot의 읽기 전용 초 단위 Timeline, Track lane, 단일 Mix playback, 실제 media duration·Playhead, seek·scroll·zoom·keyboard·Track 선택 기반을 구현했다. 실제 DB 승인은 별도 유지한다.
...
| AI-native DAW Product | [진행 중] | D0·D1 완료, D2 Timeline Playback Foundation 구현·Draft 검토; 실제 DB 전환과 Waveform·Clip·Section·Mixer·D3~D9 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
| Reviewer Authentication Authority | [부분 결정] | CURRENT `LOCAL_ONLY`, DohaMusic-only Provider 호출과 delegated trust direction 확정; production topology·reviewer population·실제 provider는 미확정 | [Authority](docs/09-security/reviewer-authentication-deployment-authority.md) |

## 현재 우선 작업

**최우선 NEXT:** Timeline Playback Foundation Draft를 검토한 뒤 Waveform / richer Playhead를 진행한다. 이후 Clip Editing Foundation, Mixer, AI Segment Editing 순으로 확장한다. 실제 사용자 DB `0017 → 0018` 전환은 별도 승인 Gate로 유지하며, Workspace Job의 Provider dispatch wiring과 background daemon은 별도 Track이다.
...
16. [부분 완료] AI-native DAW D1-A에서 `ProjectCompositionSelection`, Composition aggregate GET과 선택 PATCH를 구현했고, D1-Transition은 `NO_PREEXISTING_SELECTION_AUTHORITY`에 따라 자동 선택 없이 격리 검증했다. D1-B는 Project 상세에서 `empty`, `selection_required`, `ready`, 명시 Snapshot 선택·PATCH 후 refetch, exact AssetVersion·safe Artifact·Mix·lineage 표시를 구현했다. 실제 DB 전환과 canonical Track·Section·Clip Domain은 미구현이며 신규 `EditIntent`를 만들지 않는다.
17. [부분 결정] [ADR-037](docs/11-decisions/ADR-037-reviewer-authentication-deployment-authority.md)은 CURRENT를 local single-user/no-login으로 확인하고 product·service identity는 DohaMusic, ReviewerAuthority는 DohaAudio가 소유하며 direct Provider login을 배제한다. Production topology·semantic reviewer population과 interaction·upstream identity issuer·service/assertion credential·private store 기술은 product-owner 결정 전까지 미확정이고 실제 인증·mapping·authority·approval은 0이다.
