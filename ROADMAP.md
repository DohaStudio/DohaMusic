...
| Workspace Artifact Domain | [진행 중] | Entity·Repository·Service, 실제 DB 0012~0016, Catalog 조회·local Resolver와 Resource API 19개 완료; Catalog row 0개, ingestion·무결성 검증·나머지 45개 API·Artifact 폴더·Runtime 미구현 | [Artifact Storage 계약](docs/03-architecture/artifact-storage-contract.md) |
...
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 21개 SQLAlchemy 2.0 Workspace Entity, 별도 `ArtifactStorageLocation`, additive revision `20260806_0012`~`20260809_0016`, Workspace Repository와 [Service 소유 transaction](docs/03-architecture/workspace-service-transaction.md)까지 구현했다. 실제 사용자 DB도 `20260809_0016`·36개 Application Table이며 Catalog row는 0개다. Catalog 조회 Repository와 설정 주입형 local Resolver도 구현했지만 실제 Bootstrap·backfill·dual write·나머지 45개 Resource REST API·Frontend·Legacy 제거는 미구현이고 현재 14개 Runtime Table과 source of truth는 변경하지 않는다.
14. [완료] [Artifact Storage 계약](docs/03-architecture/artifact-storage-contract.md)과 [ADR-032](docs/11-decisions/ADR-032-artifact-storage-resolver-integrity.md)에서 `artifact://<artifact_id>`, 별도 `artifact_storage_locations` Catalog, trusted ingestion, SHA-256·size·MIME 검증, owner·retention·Range·GC 경계를 확정했다. Catalog Entity·revision `20260809_0016`·실제 사용자 DB 적용과 local Resolver를 완료했다. 다음은 trusted ingestion과 physical integrity 검증이며 Resource API는 19/64, Artifact API는 0/3을 유지한다.
