...
| Workspace Artifact·Job Domain | [진행 중] | 실제 DB 0012~0017, Job schema·Index와 Artifact·CompositionSnapshot API 완료; Job Cursor·Worker·Service state machine·API 0/5와 나머지 39개 API 미구현 | [Workspace Job Foundation](docs/03-architecture/workspace-job-foundation.md) |
...
**최우선:** revision `20260810_0017`의 Job schema·Index 실제 DB 적용과 Bootstrap exact revision Gate 동기화를 완료했다. 다음은 Job Cursor·Repository keyset·Service state machine·Worker claim/lease와 completion Unit of Work를 순차 구현하며 Job API는 이 기반 검증 전 시작하지 않는다.
