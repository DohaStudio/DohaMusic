...
| Workspace Artifact Domain | [진행 중] | Entity·Repository·Service, 실제 DB 0012~0016, Artifact API와 CompositionSnapshot 불변 생성·Cursor·Idempotency 기반 완료; Snapshot Router·destructive repair·나머지 42개 API·운영 Artifact 폴더·Runtime 미구현 | [CompositionSnapshot 기반](docs/06-api/composition-snapshot-foundation.md) |
...
12. [Workspace v1 API 계약](docs/06-api/workspace-rest-api-contract.md)은 공통 Router·응답 Schema·request ID·오류 분기, [명시적 Bootstrap 도구](docs/06-api/workspace-api-foundation-bootstrap.md), [HMAC Cursor Pagination](docs/06-api/cursor-pagination.md)과 Workspace·MusicProject·ProjectAsset·Asset·AssetVersion·Artifact Resource Endpoint 22개를 구현했다. [CompositionSnapshot 기반](docs/06-api/composition-snapshot-foundation.md)은 불변 aggregate·Owner/ProjectAsset scope·자동 version·Cursor·Idempotency까지 완료했지만 Router 3개는 미구현이다. Resource API 진행도는 22/64다.
13. [Asset 중심 목표 DB](docs/07-database/database-redesign-overview.md)는 21개 SQLAlchemy 2.0 Workspace Entity, 별도 `ArtifactStorageLocation`, additive revision `20260806_0012`~`20260809_0016`, Workspace Repository와 [Service 소유 transaction](docs/03-architecture/workspace-service-transaction.md)까지 구현했다. 실제 사용자 DB도 `20260809_0016`·36개 Application Table이며 Catalog row는 0개다. CompositionSnapshot은 기존 schema와 Index만으로 원자적 생성·keyset 조회 기반을 구현했다. 실제 Bootstrap·backfill·dual write·나머지 42개 Resource REST API·Frontend·Legacy 제거는 미구현이고 현재 14개 Runtime Table과 source of truth는 변경하지 않는다.
