...
Bootstrap 최신 실행 순서: #164/#165/#166/#167/#168 [ADR-082 Windows serialization mechanics](docs/11-decisions/ADR-082-windows-ceremony-serialization-foundation.md) merged → 별도 Draft [ADR-083 private pin facts transport/comparison](docs/11-decisions/ADR-083-private-pin-facts-reader-foundation.md) → reviewed independent private custody/designation/provenance·fresh journal/history·authoritative complete manifest/possession/currentness verification → explicit durable admission owner/same-event pin install·reconciliation → Installation/Claim·principal/binding/Auth → Rights Writer/Adapter/Runtime. private file facts/OS acquisition/public scope/helper lifetime check는 authority가 아니며 production composition unavailable 및 기존 Phase/DoD 진행률은 유지한다. 아래 직전 단계 기록의 NEXT는 이 실행 순서로 갱신한다.
