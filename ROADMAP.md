...
3. D3 `[진행 중]`: ADR-040·045·047·050·052의 WorkingComposition/Preview authority, 9개 persistence table, atomic mutation Service와 19개 Product operation, Frontend Track/Clip editing·memory Undo/Redo, exact AssetVersion-safe media source·Clip별 source-window Waveform, Working Preview와 Composition Commit Backend/Frontend integration을 구현했다. D4 Mixer·독립 Export는 `[계획]`이다.
...
| AI-native DAW Product | [진행 중] | D0·D1·D2와 D3 WorkingComposition·Preview·Composition Commit Backend/Frontend integration, Clip editing·memory Undo/Redo·safe media source·Track/Clip Waveform 완료; 실제 DB 전환·Section·Mixer·D4~D9 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
**최우선 NEXT:** Composition Commit 다음으로 Mixer 또는 잔여 D3 editing gate를 진행한다. 이후 AI Segment Editing 순서로 확장한다. MIDI·Piano Roll은 NOT IMPLEMENTED, SoundFont는 NOT INTEGRATED 상태의 별도 우선순위다. 실제 사용자 DB `0017 → 0024` 전환은 별도 승인 Gate로 유지하며, Workspace Job background daemon activation은 별도 Track이다.
...
16. [진행 중] AI-native DAW D1-A~D2와 D3 WorkingComposition persistence·Service·19개 Product API, Frontend Track/Clip UI·memory Undo/Redo, exact AssetVersion-safe media source·Track/Clip Waveform, Working Preview와 Composition Commit Backend/Frontend integration을 완료했다. expected revision CAS, revision-safe replay와 revision-pinned manifest를 소비하지만 canonical Section은 미구현이며 신규 `EditIntent`를 만들지 않았다.
