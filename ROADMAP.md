...
2. D1 `[D1-A 완료 / Transition 완료 / D1-B Draft 검토]`, D2 `[다음]`: Composition Read의 Workspace 권위·selection·projection·aggregate와 무선택 Bootstrap transition을 구현하고 Project 상세에 empty·명시 선택·ready UI를 연결했다. 실제 DB 승인은 별도 유지하며 다음은 읽기 전용 Timeline Playback 기반이다.
...
이번 D1-B에서 Frontend 코드와 테스트를 변경했다. Backend 공개 API·DB·Provider·Training·Dataset은 변경하지 않았다. Phase 8 `100%`는 로컬 MVP 판정이며 이 Track의 완료율이 아니다.
...
| AI-native DAW Product | [진행 중] | D0·D1-A·D1-Transition 완료, D1-B Frontend 구현 후 Draft 검토; 실제 DB 전환과 D2~D9 Runtime 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
| Reviewer Authentication Authority | [부분 결정] | CURRENT `LOCAL_ONLY`와 delegated trust direction 확정; ADR-038 재판정에서도 V1 production topology·reviewer population·interaction 근거가 없어 provider selection 보류 | [Authority](docs/09-security/reviewer-authentication-deployment-authority.md) |

## 현재 우선 작업

**최우선 NEXT:** D1-B Frontend Draft를 검토한 뒤 Timeline Playback Foundation을 진행한다. 실제 사용자 DB `0017 → 0018` 전환은 별도 승인 Gate로 유지하며, Workspace Job의 Provider dispatch wiring과 background daemon은 별도 Track이다.
...
17. [부분 결정] [ADR-037](docs/11-decisions/ADR-037-reviewer-authentication-deployment-authority.md)은 CURRENT local/no-login과 identity·authority 경계를 확정했다. [ADR-038](docs/11-decisions/ADR-038-v1-reviewer-authentication-product-decision.md)은 CURRENT MVP가 V1 production 승인 근거가 아니고 D8 Review Hub도 DohaAudio semantic reviewer population·interaction을 정하지 않는다고 재검증했다. V1 topology·reviewer·upstream proof와 Provider model은 product-owner 결정 전까지 미확정이고 실제 인증·mapping·authority·approval은 0이다.
