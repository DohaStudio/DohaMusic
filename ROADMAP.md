...
3. D3 `[진행 중]`: ADR-040·045·047·050·052의 WorkingComposition/Preview authority, 9개 persistence table, atomic mutation Service와 18개 Product operation, Frontend Track/Clip editing·memory Undo/Redo, exact AssetVersion-safe media source·Clip별 source-window Waveform과 Working Preview Backend/Frontend integration을 구현했다. Composition commit은 미구현이며 D4 Mixer·독립 Export는 `[계획]`이다.
...
Clip Persistence·Authority, revision-safe idempotency와 WorkingComposition atomic mutation Service·18개 Product operation을 구현했다. Frontend는 exact AssetVersion Clip create, Track/Clip mutation, response revision 기반 reconcile, same-ID restore·unsplit/resplit memory Undo/Redo, Clip별 `[source_in, source_out)` Waveform과 명시적 Working Preview action·Job polling·stale/rerender·Global Player handoff를 소비한다. Backend는 exact AssetVersion을 현재 eligible한 exactly-one Artifact와 same-origin content reference로 해석하고 revision-pinned Working Preview Job을 non-canonical Preview Asset/AssetVersion/Artifact로 완성한다. Provider·Training·Dataset·GPU·Common Contract는 변경하지 않았고 Composition commit·Section·Mixer·range selection은 구현하지 않았다. Phase 8 `100%`는 로컬 MVP 판정이며 이 Track의 완료율이 아니다.
...
| AI-native DAW Product | [진행 중] | D0·D1·D2와 D3 WorkingComposition·Preview Backend/Frontend integration, Clip editing·memory Undo/Redo·safe media source·Track/Clip Waveform 완료; Composition commit·실제 DB 전환·Section·Mixer·D4~D9 미구현 | [AI-native DAW DoD](docs/DoD/AI-Native-DAW.md) |
...
**최우선 NEXT:** Working Preview Backend/Frontend integration 다음으로 Composition commit을 진행한다. 이후 Mixer, AI Segment Editing 순서로 확장한다. MIDI·Piano Roll은 NOT IMPLEMENTED, SoundFont는 NOT INTEGRATED 상태의 별도 우선순위다. 실제 사용자 DB `0017 → 0024` 전환은 별도 승인 Gate로 유지하며, Workspace Job background daemon activation은 별도 Track이다.
...
16. [진행 중] AI-native DAW D1-A~D2와 D3 WorkingComposition persistence·Service·Product API, Frontend Track/Clip UI·memory Undo/Redo, exact AssetVersion-safe media source·Track/Clip Waveform과 Working Preview Backend/Frontend integration을 완료했다. expected revision CAS, revision-safe replay와 revision-pinned manifest를 소비하지만 Composition commit·canonical Section은 미구현이며 신규 `EditIntent`를 만들지 않았다.
