...
Bootstrap 최신 실행 순서: #164~#170 [ADR-084 designation provenance Contract](docs/11-decisions/ADR-084-designation-provenance-reader-input-contract.md) merged → 별도 Draft [ADR-085 raw record snapshot mechanics](docs/11-decisions/ADR-085-designation-record-snapshot-foundation.md) → reviewed independent human designation/provisioning source·ACL custody/authenticity/eligibility → fresh journal/history·authoritative complete manifest/possession/currentness → durable admission owner/same-event pin install·reconciliation → Installation/Claim·principal/binding/Auth → Rights Writer/Adapter/Runtime. file/digest/snapshot/helper/OS lock은 authority가 아니며 production unavailable 및 Phase/DoD 진행률은 유지한다. 아래 직전 단계 기록의 NEXT는 이 실행 순서로 갱신한다.
