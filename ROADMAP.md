...
Bootstrap 최신 실행 순서: #164 ADR-078/#165 public journal/#166 [ADR-080 handoff Contract](docs/11-decisions/ADR-080-private-admission-currentness-handoff-contract.md) merged → 별도 Draft [ADR-081 witness lifetime helper](docs/11-decisions/ADR-081-provider-witness-lifetime-foundation.md) → reviewed independent pin/journal/provenance readers·OS ceremony serialization → explicit durable admission owner/same-event pin install·reconciliation → Installation/Claim·principal/binding/Auth → Rights Writer/Adapter/Runtime. public facts/helper lifetime check는 authority가 아니며 기존 Phase/DoD 진행률은 유지한다. 아래 직전 단계 기록의 NEXT는 이 실행 순서로 갱신한다.
