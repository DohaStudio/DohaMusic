...
| Workspace Artifact·Job Domain | [진행 중] | 독립 체크리스트 | 실제 DB 0012~0017, Job schema·Index·Cursor·Repository keyset·Service state/cancel/retry·Completion UoW 기반과 Artifact·CompositionSnapshot API 완료; Worker runtime·API 0/5와 나머지 39개 API 미구현 | [Workspace Job Foundation](docs/03-architecture/workspace-job-foundation.md) |
...
Workspace Artifact·Job Domain은 Provider Runtime의 `lm`·`audio`·`vocal` 결과와 DohaMusic의 Mix·Export·Preview·Composition Snapshot·독립 Job을 분리하는 진행 중 Track이다. 목표 Workspace Entity 21개, Catalog·keyset·Workspace Job schema revision `20260806_0012`~`20260810_0017`을 실제 사용자 DB에 적용했고 Workspace Repository와 Service 소유 transaction 경계를 완료했다. Bootstrap CLI의 exact revision Gate도 source·실제 DB와 같은 `20260810_0017`로 동기화했지만 실제 Bootstrap은 실행하지 않았다. Job HMAC Cursor와 Owner·Workspace scope Repository keyset page, 생성·입력 lineage·상태·진행률·취소·재시도·멱등성 Service 및 Completion Unit of Work 기반을 구현했다. `/api/v1` Resource Endpoint는 25개다. Worker claim/lease runtime·Provider 실제 호출과 공식 Job API 5개는 미구현이다. source metadata와 실제 사용자 DB는 36개 Application Table이고 현행 Runtime Table 14개가 계속 source of truth다. 나머지 39개 Resource Endpoint·backfill·dual write·Frontend·Legacy 제거와 실제 `DohaArtifacts` 폴더·Runtime 전환은 수행하지 않았다.
