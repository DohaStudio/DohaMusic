...
| Workspace Artifact Domain | [진행 중] | 독립 체크리스트 | Entity·Repository·Service, 실제 DB 0012~0015와 Workspace·Project·ProjectAsset·Asset Resource API 16개 완료; 나머지 48개 API·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
Workspace Artifact Domain은 Provider Runtime의 `lm`·`audio`·`vocal` 결과와 DohaMusic의 Mix·Export·Preview·Composition Snapshot을 분리하는 진행 중 Track이다. 목표 21개 Entity와 additive revision `20260806_0012`, keyset Index revision `20260807_0013`~`20260808_0015`를 실제 사용자 DB에 적용했고 Workspace Repository와 Service 소유 transaction 경계를 완료했다. `/api/v1` 공통 기반, 명시적 Bootstrap 도구, HMAC Cursor Pagination과 Workspace·MusicProject·ProjectAsset·Asset Resource Endpoint 16개를 구현했다. Asset은 신뢰된 Owner 전체와 선택적 Workspace·type filter, version 1 Cursor와 keyset Repository·Service·Index를 사용하며 생성·상세·Metadata PATCH·Soft Delete를 제공한다. 별도 범용 Unit of Work는 도입하지 않았고 현행 Runtime Table 14개가 계속 source of truth다. 나머지 48개 Resource Endpoint·Idempotency replay·backfill·dual write·Frontend·Legacy 제거와 `D:/DohaArtifacts/music/{mixes,exports,previews,snapshots,runs}` 폴더·Runtime 전환은 수행하지 않았다.
