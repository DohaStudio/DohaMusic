...
| Workspace Artifact Domain | [진행 중] | 독립 체크리스트 | Entity·Repository·Service, 실제 DB 0012·0013·0014, ProjectAsset Cursor 기반과 첫 Resource API 8개 완료; ProjectAsset API·나머지 56개 API·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
Workspace Artifact Domain은 Provider Runtime의 `lm`·`audio`·`vocal` 결과와 DohaMusic의 Mix·Export·Preview·Composition Snapshot을 분리하는 진행 중 Track이다. 목표 21개 Entity와 additive revision `20260806_0012`, Workspace·Project keyset Index revision `20260807_0013`, ProjectAsset partial keyset Index revision `20260807_0014`를 실제 사용자 DB에 적용했고 Workspace Repository와 Service 소유 transaction 경계를 완료했다. `/api/v1` 공통 기반, 명시적 Bootstrap 도구, HMAC Cursor Pagination과 Workspace·MusicProject Resource Endpoint 8개를 구현했다. ProjectAsset 전용 Cursor·keyset page는 구현했지만 ProjectAsset Router는 수행하지 않았다. 다음 범위는 ProjectAsset Resource API 3개이며 이후 Asset Cursor·Index와 Resource API 5개를 진행한다. 별도 범용 Unit of Work는 도입하지 않았고 현행 Runtime Table 14개가 계속 source of truth다. 나머지 56개 Resource Endpoint·Idempotency replay·backfill·dual write·Frontend·Legacy 제거와 `D:/DohaArtifacts/music/{mixes,exports,previews,snapshots,runs}` 폴더·Runtime 전환은 수행하지 않았다.
