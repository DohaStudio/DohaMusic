...
| Workspace Artifact Domain | [진행 중] | 독립 체크리스트 | Entity·Repository·Service, 실제 DB 0012~0015와 Resource API 19개 완료; Artifact Catalog·Resolver·무결성 계약 확정, Catalog Table·나머지 45개 API·Artifact 폴더·Runtime 미구현 | [Artifact Storage 계약](docs/03-architecture/artifact-storage-contract.md) |
...
Workspace Artifact Domain은 Provider Runtime의 `lm`·`audio`·`vocal` 결과와 DohaMusic의 Mix·Export·Preview·Composition Snapshot을 분리하는 진행 중 Track이다. 목표 21개 Entity와 additive revision `20260806_0012`, keyset Index revision `20260807_0013`~`20260808_0015`를 실제 사용자 DB에 적용했고 Workspace Repository와 Service 소유 transaction 경계를 완료했다. `/api/v1` 공통 기반, 명시적 Bootstrap 도구, HMAC Cursor Pagination과 Workspace·MusicProject·ProjectAsset·Asset·AssetVersion Resource Endpoint 19개를 구현했다. [Artifact Storage 계약](docs/03-architecture/artifact-storage-contract.md)은 경로 없는 Artifact와 별도 `artifact_storage_locations` Catalog, `artifact://` URI, trusted ingestion·무결성·owner·retention·Range·GC를 승인했다. Catalog Entity·Migration·Resolver와 Artifact API 3개는 아직 미구현이며 현행 Runtime Table 14개가 계속 source of truth다. 나머지 45개 Resource Endpoint·Idempotency replay·backfill·dual write·Frontend·Legacy 제거와 `D:/DohaArtifacts/music/{mixes,exports,previews,snapshots,runs}` 폴더·Runtime 전환은 수행하지 않았다.
