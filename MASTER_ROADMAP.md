...
| F6. Guided Voice Enrollment | [진행 중] | 독립 체크리스트 | Backend·Frontend와 Windows/CI FFmpeg 통합 검증 완료, scheduler·인증·실기기 평가는 미구현 | [Frontend Roadmap](planning/frontend-roadmap.md#f6--guided-voice-enrollment-진행-중) |
...
- 현재 사실: 기존 단일 WAV upload를 유지하면서 신규 Enrollment 7개 endpoint, 최대 10개 sample, PCM16 48kHz mono 정규화, 임시·최종 Storage, 기본 peak·RMS·silence·clipping, 멱등성, 24시간 sliding/7일 absolute lazy expiration을 구현했다. `/voice`에는 8단계 Wizard, MediaRecorder·입력 수준·preview, WAV/WebM/Ogg upload, 품질 확인·대표 선택·submit, session 복원·취소·만료와 Studio 선택을 연결했다. Windows FFmpeg 8.1.2와 Ubuntu·Windows CI에서 합성 WebM/Ogg Opus 정규화, 손상 입력, 미설치·timeout·비정상 종료·부분 출력 cleanup을 검증한다.
...
- 남은 gap: 주기적 expiration·cleanup scheduler와 crash 복구, chunk resume, 동의 철회·파생 삭제, FFmpeg build 라이선스 운영 검토, 실제 사용자 수동 녹음·브라우저 MIME 평가.
