...
| Workspace Artifact Domain | [진행 중] | 독립 체크리스트 | Entity·Repository·Service, 실제 DB 0012·0013, ProjectAsset Cursor·0014 source 기반과 첫 Resource API 8개 완료; 0014 실제 적용·나머지 56개 API·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
Workspace Artifact Domain은 Provider Runtime의 `lm`·`audio`·`vocal` 결과와 DohaMusic의 Mix·Export·Preview·Composition Snapshot을 분리하는 진행 중 Track이다. 목표 21개 Entity와 additive revision `20260806_0012`, Workspace·Project keyset Index revision `20260807_0013`을 실제 사용자 DB에 적용했고 Workspace Repository와 Service 소유 transaction 경계를 완료했다. `/api/v1` 공통 기반, 명시적 Bootstrap 도구, HMAC Cursor Pagination과 Workspace·MusicProject Resource Endpoint 8개를 구현했다. ProjectAsset 전용 Cursor·keyset page와 partial Index source revision `20260807_0014`는 임시 SQLite 검증까지 완료했지만 실제 사용자 DB 적용과 ProjectAsset Router는 수행하지 않았다. 별도 범용 Unit of Work는 도입하지 않았고 현행 Runtime Table 14개가 계속 source of truth다. 나머지 56개 Resource Endpoint·Idempotency replay·backfill·dual write·Frontend·Legacy 제거와 `D:/DohaArtifacts/music/{mixes,exports,previews,snapshots,runs}` 폴더·Runtime 전환은 수행하지 않았다.
