...
| Workspace Artifact Domain | [진행 중] | 독립 체크리스트 | Entity·실제 DB additive migration·Repository·Service·HMAC Cursor·keyset Index 실제 적용과 첫 Resource API 8개 완료, 나머지 56개 API·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
Workspace Artifact Domain은 Provider Runtime의 `lm`·`audio`·`vocal` 결과와 DohaMusic의 Mix·Export·Preview·Composition Snapshot을 분리하는 진행 중 Track이다. 목표 21개 Entity와 additive revision `20260806_0012`, keyset 복합 Index revision `20260807_0013`을 실제 사용자 DB에 적용했고 Workspace Repository와 Service 소유 transaction 경계를 완료했다. `/api/v1` 공통 기반, 명시적 Bootstrap 도구, HMAC Cursor Pagination과 Workspace·MusicProject Resource Endpoint 8개를 구현했다. 별도 범용 Unit of Work는 현재 단일 DB·단일 Session 구조에 필요하지 않아 도입하지 않았다. 실제 Bootstrap은 수행하지 않아 신규 Table은 비어 있고 현행 Runtime Table 14개가 계속 source of truth다. 나머지 56개 Resource Endpoint·Idempotency replay·backfill·dual write·Frontend·Legacy 제거와 `D:/DohaArtifacts/music/{mixes,exports,previews,snapshots,runs}` 폴더·Runtime 전환은 수행하지 않았다. Phase 5와 Phase 8의 기존 완료 상태도 변경하지 않는다.
