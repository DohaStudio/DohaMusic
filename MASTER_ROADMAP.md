...
Vocal rights 독립 Track: merged #160 [ADR-075](docs/11-decisions/ADR-075-dohavocal-production-rights-domain-decision.md)의 V1에 따라 SQLite Persistence Foundation 11 tables·immutable ledger/evidence·current projection·actual guard UPDATE·final receipt persistence·additive `20260918_0036`를 구현했다. 후속 `20260918_0037`은 기본 SQLite 설정에서 ScopeGuard REPLACE/epoch reset을 차단하는 trigger-only integrity 수정이며 PR #161과 함께 merged다. 이는 운영 권한 시스템 완료가 아니다. `MINIMAL_PORT_ADAPTATION_REQUIRED`에 따른 Completion port/audit·production adapter, authenticated writer/evidence·auth·ownership writer 협력·rollout/실제 DB 적용은 미구현/미수행이다. [ADR-076](docs/11-decisions/ADR-076-product-deployment-bootstrap-authority.md)은 #162 채택/merged된 external deployment root와 최초 binding bootstrap Decision이다. 다음 최소 unit [ADR-077 verifier](docs/03-architecture/bootstrap-issuance-integrity-verifier.md)은 별도 Foundation Draft 구현·검증 중이며 issuance 무결성만 확인하고 권한을 부여하지 않는다. NEXT는 이 Draft 별도 검증/merge 후 Installation/Bootstrap Persistence·principal identity/binding·authentication 통합이다. trusted provisioning/current status/journal/persistence/WebAuthn/principal/binding/Recovery/Transfer/Evidence/Writer는 미구현이며 기존 Phase·DoD 체크·진행률은 변경하지 않는다.
...
| Phase B New Implementation Separation | [진행 중] | DohaVocal `0.1.0` 호환·`0.2.0` payload consumer DTO/trust/transient acquisition·Trusted Payload resolver, durable locator·verified staging·acquisition·Completion Foundation 구현; reclaim·concrete wiring·production rights adapter·실제 model 미구현 |
