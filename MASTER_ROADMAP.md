...
| Workspace Artifact Domain | [진행 중] | 독립 체크리스트 | Entity·Repository·Service, 실제 DB 0012~0016, Catalog·Resolver·Trusted Ingestion·owner/retention read Gate·dry-run reconciliation과 Resource API 19개 완료; destructive repair·나머지 45개 API·운영 Artifact 폴더·Runtime 미구현 | [Artifact Storage 계약](docs/03-architecture/artifact-storage-contract.md) |
...
Workspace Artifact Domain은 Provider Runtime의 `lm`·`audio`·`vocal` 결과와 DohaMusic의 Mix·Export·Preview·Composition Snapshot을 분리하는 진행 중 Track이다. 목표 Workspace Entity 21개와 additive revision `20260806_0012`, keyset Index revision `20260807_0013`~`20260808_0015`, Catalog revision `20260809_0016`을 실제 사용자 DB에 적용했고 Workspace Repository와 Service 소유 transaction 경계를 완료했다. `/api/v1` 공통 기반, HMAC Cursor와 Workspace·MusicProject·ProjectAsset·Asset·AssetVersion Resource Endpoint 19개를 구현했다. source metadata와 실제 사용자 DB는 36개 Application Table이며 Catalog row는 0개다. Catalog 조회·local Resolver와 trusted ingestion에 이어 Owner 계보·retention matrix·full SHA-256 content read Gate와 승인 namespace의 batch dry-run reconciliation을 격리 구현했다. destructive repair·Range·Artifact API 3개는 미구현이다. 현행 Runtime Table 14개가 계속 source of truth이고 나머지 45개 Resource Endpoint·backfill·dual write·Frontend·Legacy 제거와 실제 `DohaArtifacts` 폴더·Runtime 전환은 수행하지 않았다.
