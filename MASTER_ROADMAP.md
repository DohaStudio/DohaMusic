...
| Workspace Artifact Domain | [진행 중] | 독립 체크리스트 | Entity·additive migration·DB preflight 도구 구현, 실제 DB·Artifact 폴더·Runtime 미전환 | [Workspace Artifact 모델](docs/03-architecture/workspace-artifact-model.md) |
...
Workspace Artifact Domain은 Provider Runtime의 `lm`·`audio`·`vocal` 결과와 DohaMusic의 Mix·Export·Preview·Composition Snapshot을 분리하는 진행 중 Track이다. 목표 21개 Entity, additive revision과 실제 적용 전 read-only Preflight·backup 도구까지 구현했지만 실제 사용자 DB, `D:/DohaArtifacts/music/{mixes,exports,previews,snapshots,runs}` 폴더, Repository·API·Runtime은 변경하지 않았다. Phase 5와 Phase 8의 기존 완료 상태도 변경하지 않는다.
